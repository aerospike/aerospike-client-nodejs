...
import type { Client, BatchResult, AerospikeRecord, BatchSelectRecord, Key } from '../lib/aerospike.js';
...
describe('client.batchSelect()', function () {
  ...
  it('should fail reading bins from non-existent records', function (done) {
    ...
    const bins: string[] = ['i', 's']
    client.batchSelect(keys, bins, function (err?: Error, results?: BatchSelectRecord[]) {
      expect(err).not.to.be.ok
      expect(results?.length).to.equal(numberOfRecords)
      results?.forEach(function (result: BatchSelectRecord) {
  ...
  it('returns an empty array when no keys are passed', () => {
    client.batchSelect([], ['i'])
      .then((results: BatchSelectRecord[]) => expect(results).to.eql([]))
  })
})
