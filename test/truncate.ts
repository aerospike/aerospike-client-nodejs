...
/* eslint-env mocha */
import Aerospike, { Client, BatchResult, AerospikeRecord, Key } from '../lib/aerospike.js';
...
describe('client.truncate() #slow', function () {
  ...
  // of the record keys.
  function genRecords (kgen: () => Key, noRecords: any) {
    const generators = {
      keygen: kgen,
  ...
  async function checkRecords (truncated: any, remaining: any, pollInt: number): Promise<any> {
    const results: any = await client.batchRead((truncated || []).concat(remaining || []));
    for (const result of results) {
      const expectExist = !!remaining.find((record: AerospikeRecord) => record.key.equals(result.record.key))
  ...
  it('deletes all records in the set', async function () {
    const ns: string = helper.namespace
    const set: string = setgen()
    const noRecords: number = 5
    const pollIntMs: number = 10 // Poll interval in ms to check whether records have been removed

    const kgen: () => Key = keygen.string(ns, set, { prefix: 'test/trunc/', random: false })
    const records: AerospikeRecord[] = await genRecords(kgen, noRecords)
    await sleep(5)
