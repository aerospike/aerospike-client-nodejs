...
var LargeList = function LargeList (key, binName, writePolicy, createModule) {
  ...
  instance.add = function add (val, callback) {
  ...
  instance.update = function update (val, callback) {
    if (Array.isArray(arguments[0])) {
  ...
  instance.remove = function remove (val, callback) {
  ...
  instance.filter = function filter (filterArgs, callback) {
    this.executeLDTFunction('filter', arguments, 1, 0)
  }

  instance.findRange = function findRange (valBegin, valEnd, filterArgs, callback) {
    var arglength = arguments.length
  ...
  instance.scan = function scan (callback) {
    this.executeLDTFunction('scan', arguments, 1)
  }

  instance.destroy = function destroy (callback) {
    this.executeLDTFunction('destroy', arguments, 1)
  }

  instance.size = function size (callback) {
    this.executeLDTFunction('size', arguments, 1)
  }

  instance.getConfig = function getConfig (callback) {
