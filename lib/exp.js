...
const cdtContext = require('./cdt_context')
const exp = as.exp
const writeFlags = as.expWriteFlags
const readFlags = as.expReadFlags
const pathSelectFlags = as.pathSelectFlags
const pathModifyFlags = as.pathModifyFlags
 ...
 * @return {AerospikeExp}
 */
exports.unknown = () => [{ op: exp.ops.UNKNOWN, count: 1 }]
 ...
 * <code>bin</code> argument to {@link module:aerospike/exp.vectorDist}.
 *
 * @function
 * @param {string} binName Bin name.
 * @return {AerospikeExp} vector bin
 *
 * @since v6.next
 */
exports.binVector = _binTypeExp(exp.type.VECTOR)

/**
 * @summary Distance metric for {@link module:aerospike/exp.vectorDist}.
 *
 * @enum {number}
 *
 * @property {number} EUCLIDEAN - Squared Euclidean (L2) distance. Smaller
 * values indicate closer vectors - the natural sort direction for
 * {@link Query#orderBy} is therefore {@link module:aerospike/query.order|ASC}.
 * @property {number} DOT_PRODUCT - Dot product. Larger values indicate
 * closer vectors - natural sort direction is
 * {@link module:aerospike/query.order|DESC}.
 * @property {number} COSINE - Cosine similarity (not distance - there is no
 * separate "cosine distance" metric). Larger values indicate closer
 * vectors - natural sort direction is
 * {@link module:aerospike/query.order|DESC}.
 *
 * @since v6.next
 */
exports.vectorDistanceMetric = {
  EUCLIDEAN: 0,
  DOT_PRODUCT: 1,
  COSINE: 2
}

const _vectorDistOp = (metric) => {
  switch (metric) {
    case exports.vectorDistanceMetric.EUCLIDEAN:
      return exp.ops.VECTOR_EUCLIDEAN_DIST
    case exports.vectorDistanceMetric.DOT_PRODUCT:
      return exp.ops.VECTOR_DOT_PRODUCT
    case exports.vectorDistanceMetric.COSINE:
      return exp.ops.VECTOR_COSINE_SIM
    default:
      throw new TypeError(`metric must be one of Aerospike.exp.vectorDistanceMetric; got ${metric}`)
  }
}

/**
 * Create expression that returns the distance between a stored vector bin
 * and a query vector as a 64-bit float, using the given distance metric.
 * The query vector's element type and dimension count must match the
 * stored vector, or the expression evaluates to 'unknown'.
 *
 * @function
 * @param {number} metric - One of {@link module:aerospike/exp.vectorDistanceMetric}.
 * @param {Vector} query - Query vector, compared against the stored bin.
 * Sent as its complete serialized wire value ({@link Vector#toBuffer}),
 * matching what {@link module:aerospike/exp.binVector} reads back on the
 * stored side.
 * @param {AerospikeExp} bin - Vector bin expression, typically
 * {@link module:aerospike/exp.binVector}.
 * @return {AerospikeExp} float value - the distance/similarity score.
 *
 * @since v6.next
 *
 * @example <caption>Cosine similarity between vector bin "embedding" and a query vector > 0.8</caption>
 *
 * const Aerospike = require('aerospike')
 * const exp = Aerospike.exp
 *
 * const query = Aerospike.Vector.ofFloat32([0.1, 0.2, 0.3])
 * const filterExpression = exp.gt(
 *   exp.vectorDist(exp.vectorDistanceMetric.COSINE, query, exp.binVector('embedding')),
 *   exp.float(0.8))
 */
exports.vectorDist = (metric, query, bin) => {
  const queryBuffer = query.toBuffer()
  return [
    { op: _vectorDistOp(metric), count: 3 },
    ...exports.bytes(queryBuffer, queryBuffer.length),
    ...bin
  ]
}
...
/**
 * Answers true if the value of left argument is contained in list specified by
 * the right argument.
 *
 * @param {AerospikeExp} left Value expression to test for membership.
 * @param {AerospikeExp} right Expression that evaluates to a list of values to search.
 * @return {AerospikeExp} - boolean value
 */
exports.inList = _cmpExp(exp.ops.IN_LIST)

/**
 * Return a remove_result object to indicate entry deletion for modifyByPath.
 *
 * @return {AerospikeExp} - nothing
 */
exports.removeResult = () => [{ op: exp.ops.REMOVE_RESULT, count: 1 }]

/**
 * Return a remove_result object to indicate entry deletion for modifyByPath.
 * This is a convenient alias for {@link removeResult}.
 *
 * @return {AerospikeExp} - nothing
 */
exports.resultRemove = () => [{ op: exp.ops.REMOVE_RESULT, count: 1 }]

/**
 * Return a list of keys from a map-valued subexpression.
 *
 * @param {AerospikeExp} map Map-valued expression (e.g. bin or constant).
 * @return {AerospikeExp} - list value
 */
exports.mapKeys = (map) => [
  { op: exp.ops.MAP_KEYS, count: 2 },
  ...map
]
 ...
 * @return {AerospikeExp} - list value
 */
exports.mapValues = (map) => [
  { op: exp.ops.MAP_VALUES, count: 2 },
  ...map
]
 ...
 *
 * @function
 * @param {exp.loopVarPart} varId - loop variable part.
 * @return {AerospikeExp} loop variable expression.
 */
...
/**
 * String expression loop variable.
 * Used in path expressions.
 *
 * @function
 * @param {exp.loopVarPart} varId - loop variable part.
 ...
 * @param {exp.type} valueType - expression value type.
 * @param {exp.pathSelectFlags} flags - path select flags.
 * @param {cdt.Context} ctx - CDT Context.
 * @return {AerospikeExp} path select expression.
 */
exports.selectByPath = (bin, valueType, flags, ctx) => {
  if (!(ctx instanceof cdtContext)) {
    throw new TypeError('ctx must be a CDT Context')
  }
 ...
 * @param {string} bin - bin name.
 * @param {exp.type} valueType - expression value type.
 * @param {AerospikeExp} modExp - expression for modification.
 * @param {exp.pathSelectFlags} flags - path modify flags.
 * @param {cdt.Context} ctx - CDT Context.
 * @return {AerospikeExp} path modify expression.
 */
