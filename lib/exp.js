 ...
 *
 * @param mod Divisor used to divide the digest to get a remainder.
 ...
 * expression.
 *
 * @param options POSIX regex flags defined in regex.h.
 * @param regex POSIX regex string.
 ...
 * Requires server version 5.6.0+.
 *
 * @param ... Variable number of expression def followed by a scoped
 *  expression.
...
function getListType (type, returnType, isMulti) {
  ...
  switch (returnType & ~lists.returnType.INVERTED) {
    ...
    default:
      console.log("getListType valueType:"+type+"returnType:"+returnType+" FAILED")
  ...
  console.log("getListType2 valueType:"+type+"returnType:"+returnType+" FAILED")
...
function getMapType (type, returnType, isMulti) {
  ...
  switch (returnType & ~maps.returnType.INVERTED) {
    ...
    default:
      console.log("getMapType valueType:"+type+"returnType:"+returnType+" FAILED")
      return exp.type.ERROR
  }
  ...
  console.log("getMapType2 valueType:"+type+"returnType:"+returnType+" FAILED")
 ...
 * @param mhBitCount Number of min hash bits. Must be between 4 and 51 inclusive.
 * @param bin  A bin expression to apply this function to.
 * @return (hll bin) Returns the resulting hll bin after adding elements from list.
 */
 ...
 *
 * @param bin A bin expression to apply this function to.
 * @return (list bin) A list containing the index_bit_count and minhash bit count.
 */
  describe: (bin) => [
 ...
 * @param bin A bin expression to apply this function to.
 * @return (integer bin) 1 bin contains all of list, 0 otherwise.
 */
  mayContain: (bin, list) => [
