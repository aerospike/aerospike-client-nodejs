...
/**
 * True if the left expression value appears in the list expression (server 7.4+).
 *
 * @param {AerospikeExp} left
 * @param {AerospikeExp} list
 * @return {AerospikeExp}
 */
exports.inList = (left, list) => [
  { op: exp.ops.IN_LIST, count: 3 },
  ...left,
  ...list
]

/**
 * List of map keys from a map-valued expression (server 7.4+).
 *
 * @param {AerospikeExp} mapExp
 * @return {AerospikeExp}
 */
exports.mapKeys = (mapExp) => [
  { op: exp.ops.MAP_KEYS_IN, count: 2 },
  ...mapExp
]

/**
 * List of map values from a map-valued expression (server 7.4+).
 *
 * @param {AerospikeExp} mapExp
 * @return {AerospikeExp}
 */
exports.mapValues = (mapExp) => [
  { op: exp.ops.MAP_VALUES_IN, count: 2 },
  ...mapExp
]
...
/**
 * The {@link module:aerospike/exp/string|aerospike/exp/string} module defines functions
 * for string read/modify expressions (server 8.1.3+).
 *
 * @summary String expressions.
 */
exports.string = require('./exp_string')
