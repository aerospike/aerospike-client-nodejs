...
class BatchReadRecord extends BatchRecord {
  /** @private */
  constructor (key, ops, binnames, readallbins = False, policy = None) {
    super(key, None, None, false, false, batchType.BATCH_READ)
    this.ops = ops
...
class BatchWriteRecord extends BatchRecord {
  /** @private */
  constructor (key, ops, policy = None) {
    super(key, None, None, true, false, batchType.BATCH_WRITE)
    this.ops = ops
    this.policy = policy
  }
}
...
class BatchApplyRecord extends BatchRecord {
  /** @private */
  constructor (key, module, func, args, policy = None) {
    super(key, None, None, false, false, batchType.BATCH_APPLY)
    this.module = module
    this.func = func
    this.args = args
    this.policy = policy
...
class BatchRemoveRecord extends BatchRecord {
  /** @private */
  constructor (key, policy = None) {
    super(key, None, None, false, false, batchType.BATCH_REMOVE)
    this.policy = policy
  }
}
