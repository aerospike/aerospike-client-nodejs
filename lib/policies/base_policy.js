...
class BasePolicy {
  constructor (props) {
     ...
     *
     * When enabled and the server returns error details, {@link AerospikeError#subcode}
     * will contain the numeric subcode and the error message may contain a
     * server-authored detail string.
     *
     * Requires Aerospike Server version >= 8.1.3.
     *
     * @type number
     * @default 0
     * @see {@link module:aerospike/errorDetailVerbosity} for verbosity level constants.
     */
    this.errorDetailVerbosity = props.errorDetailVerbosity

    this.txn = props.txn

    this.connectTimeout = props.connectTimeout

    this.timeoutDelay = props.timeoutDelay
  }
}
