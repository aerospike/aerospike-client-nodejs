 ...
 * declare a Top-K global order-by/limit clause on a foreground {@link
 * Query}.
 *
 * Unlike {@link indexType}/{@link indexDataType}, these values are
 * currently purely client-side - they have no `as_query` wire-protocol
 * counterpart yet (Top-K is implemented as a client-side reduce layered on
 * top of an ordinary query - see
 * <code>docs/design/vector-phase-1-client-design.md</code> §4.3.3/§4.5 in
 * this repository for the full design rationale), so they are defined here
 * as plain JS objects rather than being backed by the native add-on.
 *
