 ...
 * })
 */
MapOperation.prototype.andReturn = function (returnType) {
 ...
 * })
 */
MapOperation.prototype.withContext = function (contextOrFunction) {
 ...
 * @property {number} UNORDERED_MAP - Return an ordered map.
 * @property {number} ORDERED_MAP - Return an unordered map.
 */
...
exports.setPolicy = function (bin, policy) {
  const op = new MapOperation(opcodes.MAP_SET_POLICY, bin)
  op.policy = policy
  return op
}
...
exports.put = function (bin, key, value, policy) {
  const op = new MapOperation(opcodes.MAP_PUT, bin)
  op.key = key
  op.value = value
  op.policy = policy
  return op
...
exports.putItems = function (bin, items, policy) {
  const op = new MapOperation(opcodes.MAP_PUT_ITEMS, bin)
  op.items = items
  op.policy = policy
  return op
}
 ...
 * @param {number} incr - The value to increment the map entry by. Use negative
 * value to decrement map entry.
 * @param {MapPolicy} [policy] - The map policy.
 * @returns {Object} Operation that can be passed to the {@link Client#operate} command.
 */
exports.increment = function (bin, key, incr, policy) {
  const op = new MapOperation(opcodes.MAP_INCREMENT, bin)
  op.key = key
  op.incr = incr
  op.policy = policy
  return op
...
exports.decrement = function (bin, key, decr, policy) {
  const op = new MapOperation(opcodes.MAP_DECREMENT, bin)
  op.key = key
  op.decr = decr
  op.policy = policy
  return op
...
exports.removeByKey = function (bin, key, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_KEY, bin)
  op.key = key
...
exports.removeByKeyList = function (bin, keys, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_KEY_LIST, bin)
  op.keys = keys
  op.returnType = returnType
  return op
}
...
exports.removeByKeyRange = function (bin, begin, end, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_KEY_RANGE, bin)
  op.begin = begin
  op.end = end
  op.returnType = returnType
  return op
...
exports.removeByValue = function (bin, value, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_VALUE, bin)
  op.value = value
  op.returnType = returnType
  return op
}
...
exports.removeByValueList = function (bin, values, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_VALUE_LIST, bin)
  op.values = values
  op.returnType = returnType
  return op
}
...
exports.removeByValueRange = function (bin, begin, end, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_VALUE_RANGE, bin)
  op.begin = begin
  op.end = end
  op.returnType = returnType
  return op
...
exports.removeByIndex = function (bin, index, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_INDEX, bin)
  op.index = index
  op.returnType = returnType
  return op
}
...
exports.removeByIndexRange = function (bin, index, count, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_INDEX_RANGE, bin)
  op.index = index
  op.count = count
  op.returnType = returnType
  return op
...
exports.removeByRank = function (bin, rank, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_RANK, bin)
  op.rank = rank
  op.returnType = returnType
  return op
}
...
exports.removeByRankRange = function (bin, rank, count, returnType) {
  const op = new MapOperation(opcodes.MAP_REMOVE_BY_RANK_RANGE, bin)
  op.rank = rank
  op.count = count
  op.returnType = returnType
  return op
...
exports.getByKey = function (bin, key, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_KEY, bin)
  op.key = key
  op.returnType = returnType
  return op
}
...
exports.getByKeyRange = function (bin, begin, end, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_KEY_RANGE, bin)
  op.begin = begin
  op.end = end
  op.returnType = returnType
  return op
...
exports.getByValue = function (bin, value, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_VALUE, bin)
  op.value = value
  op.returnType = returnType
  return op
}
...
exports.getByValueRange = function (bin, begin, end, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_VALUE_RANGE, bin)
  op.begin = begin
  op.end = end
  op.returnType = returnType
  return op
...
exports.getByIndex = function (bin, index, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_INDEX, bin)
  op.index = index
  op.returnType = returnType
  return op
}
...
exports.getByIndexRange = function (bin, index, count, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_INDEX_RANGE, bin)
  op.index = index
  op.count = count
  op.returnType = returnType
  return op
...
exports.getByRank = function (bin, rank, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_RANK, bin)
  op.rank = rank
  op.returnType = returnType
  return op
}
...
exports.getByRankRange = function (bin, rank, count, returnType) {
  const op = new MapOperation(opcodes.MAP_GET_BY_RANK_RANGE, bin)
  op.rank = rank
  op.count = count
  op.returnType = returnType
  return op
