...
exports.pop = function (bin, index) {
  const op = new ListOperation(opcodes.LIST_POP, bin)
  op.index = index
  return op
}
...
exports.popRange = function (bin, index, count) {
  const op = new ListOperation(opcodes.LIST_POP_RANGE, bin)
  op.index = index
  op.count = count
  return op
}
...
exports.remove = function (bin, index) {
  const op = new ListOperation(opcodes.LIST_REMOVE, bin)
  op.index = index
  return op
}
...
exports.removeRange = function (bin, index, count) {
  const op = new ListOperation(opcodes.LIST_REMOVE_RANGE, bin)
  op.index = index
  op.count = count
  return op
}
...
exports.trim = function (bin, index, count) {
  const op = new ListOperation(opcodes.LIST_TRIM, bin)
  op.index = index
  op.count = count
  return op
}
...
exports.get = function (bin, index) {
  const op = new ListOperation(opcodes.LIST_GET, bin)
  op.index = index
  return op
}
...
exports.getRange = function (bin, index, count) {
  const op = new ListOperation(opcodes.LIST_GET_RANGE, bin)
  op.index = index
  op.count = count
  return op
}
 ...
 * Every item must be a string. An empty list yields an empty string; a
 * single-item list yields that item with no separator applied. Pass
 * `separator` to place it between consecutive items (same as
 * {@link module:aerospike/lists.joinSeparator}). Inverse of
 * {@link module:aerospike/strings.splitSeparator}. Requires server 8.1.3+.
 *
 * @param {string} bin - The name of the bin. The bin must contain a List value.
 * @param {string} [separator] - Optional separator placed between consecutive items.
 * @returns {ListOperation} Operation that can be passed to the {@link Client#operate} command.
 */
exports.join = function (bin, separator) {
  if (separator === undefined) {
    return new ListOperation(opcodes.LIST_JOIN, bin)
  }
  return new ListOperation(opcodes.LIST_JOIN_SEPARATOR, bin, { separator })
}

/**
 * Concatenate the string items of a list, placing `separator` between consecutive
 * items, and return a single string. Same as {@link module:aerospike/lists.join}
 * with a separator. Requires server 8.1.3+.
 *
 * @param {string} bin - The name of the bin. The bin must contain a List value.
 * @param {string} separator - Separator placed between consecutive items.
 * @returns {ListOperation} Operation that can be passed to the {@link Client#operate} command.
 */
exports.joinSeparator = function (bin, separator) {
  return exports.join(bin, separator)
}
