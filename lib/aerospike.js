 ...
 * similarity search. Vectors need to be wrapped in the {@link Vector} class
 * so that the client can distinguish them from other types of objects.
 *
 * <code>Vector</code> bin values are sent to/received from the Aerospike
 * server as an <code>as_bytes</code> value tagged
 * <code>AS_BYTES_VECTOR</code> - see
 * `docs/design/vector-phase-1-client-design.md` in this repository for the
 * full wire format and design rationale.
 *
 * @summary {@link Vector} class
 */
exports.Vector = require('./vector')
// Register the Vector constructor with the native conversion layer so that
// record bins/CDT elements tagged AS_BYTES_VECTOR are automatically
// deserialized back into Vector instances (and vice versa on write).
as.registerVectorType(exports.Vector)
...
/**
 * The {@link module:aerospike/query|query} module defines enumerations
 * used by {@link Query#orderBy} and {@link Query#topK} to declare a
 * Top-K global order-by/limit clause on a foreground {@link Query}.
 *
 * @summary {@link module:aerospike/query|aerospike/query} module
 *
 * @since v6.next
 */
exports.query = require('./query_enums')
