...
Aerospike.prototype.connect = function connect (config) {
...
//     }
//     // er.code: compat
//     return cb(null, nullrec, meta, er.code)
//   })
// }
...
Aerospike.prototype.add = function add (key, bins, metadata, policy, cb) {
  this._currentClient.add(key, bins, metadata, policy, function addCb (er, record, metadata, key) {
    ...
    if (er.code != this.status.AEROSPIKE_OK) {
      return cb(er)
    }
...
Aerospike.prototype.connect = function connect (cb) {
