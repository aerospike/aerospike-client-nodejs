...
/**
 * The {@link module:aerospike/strings|strings} module defines string bin
 * operations for {@link Client#operate} (server 8.1.3+).
 *
 * @summary {@link module:aerospike/strings|aerospike/strings} module
 */
const strings = exports.strings = require('./strings')
if (as.string) {
  Object.keys(as.string).forEach((key) => {
    strings[key] = as.string[key]
  })
}
...
/**
 * The {@link module:aerospike/errorDetailVerbosity|errorDetailVerbosity} module
 * contains error detail verbosity levels for {@link BasePolicy#errorDetailVerbosity}.
 *
 * @summary {@link module:aerospike/errorDetailVerbosity|aerospike/errorDetailVerbosity} module
 */
exports.errorDetailVerbosity = require('./error_detail_verbosity')

/**
 * The {@link module:aerospike/subcode|subcode} module contains server error
 * subcode constants paired with parent status codes.
 *
 * @summary {@link module:aerospike/subcode|aerospike/subcode} module
 */
exports.subcode = require('./subcode')
...
exports.HyperLogLog = require('./hyperloglog')
 ...
 * @property {number} INTEGER - For use with server versions 8.1.3+. The Aerospike C client
 * maps this to <code>NUMERIC</code> on older servers.
 * @property {number} NUMERIC - <b>Deprecated.</b> For use with server versions prior to 8.1.3.
 * Use {@link module:aerospike.indexDataType.INTEGER} for server 8.1.3+. The C client maps this
 * to <code>INTEGER</code> on server 8.1.3+.
 * @property {number} GEO2DSPHERE - Values contained in the SI are GeoJSON values (points or polygons).
 * @property {number} BLOB - Values contained in the SI are blobs (Buffer in Node.js).
 * @property {number} DEFAULT - When creating set indices, the specific data type doesn't matter.
 ...
 * @property {number} SET - Set index inside a namespace.
 *
 * @see {@link Client#createIndex}
 */
...
exports.wrapHLL = function (enable) {
  as.wrapHLL(enable)
}
