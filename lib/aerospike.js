...
const isOk = as.status.AEROSPIKE_OK
...
Client.prototype.connect = function clientConnect (cb) {
  this._client.connect(function (err) {
    if (err && err.code !== isOk) {
      cb(err)
    } else {
      cb(null, this)
    }
...
Aerospike.prototype.client = function client (config) {
  let client = new Client(config)
  if (client === null) {
    throw new Error('Client object creation failed - null value returned')
  }
  this._currentClient = client._client
  return this
}

Aerospike.prototype.connect = function connect (config, cb) {
  if (!this._currentClient || typeof cb === 'function') {
    this._currentClient = this.client(config)
  }

  if (this._currentClient && typeof config === 'function') {
    cb = config
  }
...
Aerospike.prototype.createGeo2DSphereIndex = function createGeo2DSphereIndex (options, cb) {
  var policy
  var set
  if (options && options.policy) {
    policy = options.policy
...
Aerospike.prototype.info = function info (request, host, cb) {
  if (typeof host === 'function') {
    cb = host
  }
  // FIXME: currently passing policy is not posibble because of segfault
  this._currentClient.info(request, host, function infoCb (er, response, host) {
    if (er.code !== isOk) {
      return cb(er)
    }
...
Aerospike.prototype.LargeList = LargeList
...
Aerospike.prototype.query = function query (ns, set, options) {
  ...
  var queryObj = this._currentClient.query(ns, set, options)

  queryObj.execute = execute
  queryObj.Info = function info (scanId, callback) {
    this.queryInfo(scanId, callback)
  }
...
Aerospike.prototype.udfRegister = function udfRegister (udfModule, udfType, policy, cb) {
  ...
  this._currentClient.udfRegister(udfModule, udfType, policy, function udfRegisterCb (er) {
    if (er.code !== isOk) {
      return cb(er)
    }
    return cb(null)
...
Aerospike.prototype.udfRemove = function udfRemove (udfModule, policy, cb) {
  if (typeof policy === 'function') cb = policy
  this._currentClient.udfRemove(udfModule, policy, function udfRemoveCb (er) {
    if (er.code !== isOk) {
      return cb(er)
    }
    return cb(null)
