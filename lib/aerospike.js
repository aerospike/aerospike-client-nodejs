...
execute = function() {
	...
	if(self.isQuery && !self.hasUDF && !self.hasAggregation) // query 
	{
		self.foreach(onRecord, onError, onEnd);
	}
	else if( self.isQuery && self.hasAggregation) // query aggregation
	{
		self.foreach(onResult, onError, onEnd);
	}
	else if( !self.isQuery && self.hasAggregation) // scan aggregation
	{
		self.foreach(onResult, onError, onEnd);
    }
...
query = function(ns, set, options) {
  ...
  if(queryObj.isQuery == true && queryObj.hasUDF == false)
  {
	  queryObj.setScanQueryAPI(as.scanQueryAPI.QUERY);
  }

  if(queryObj.isQuery == true && queryObj.hasAggregation == true)
  {
	  queryObj.setScanQueryAPI(as.scanQueryAPI.QUERYAGGREGATION);
  }

  if(queryObj.isQuery == true && queryObj.hasUDF == true)
  {
	  queryObj.setScanQueryAPI(as.scanQueryAPI.QUERYUDF);
  }

  if(queryObj.isQuery == false && queryObj.hasUDF == false)
  {
	  queryObj.setScanQueryAPI(as.scanQueryAPI.SCAN);
  }

  if(queryObj.isQuery == false && queryObj.hasUDF == true)
  {
	  queryObj.setScanQueryAPI(as.scanQueryAPI.SCANUDF);
  }

  if(queryObj.isQuery == false && queryObj.hasAggregation == true)
  {
	  queryObj.setScanQueryAPI(as.scanQueryAPI.SCANAGGREGATION);
  }
