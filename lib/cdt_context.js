 ...
 *
 * @description Nested list/map values are limited to **64** levels of nesting.
 * The C client (7.6.1, AER-6957 / CVE-2026-63662) rejects deeper msgpack on
 * deserialize; this matches the server stored-value bound.
 *
...
class CdtContext {
   ...
   *   `exp.and(...)` with a single call to `addAndFilter()`.
   *
   * - The preceeding context entry must not be an expression type;
   *   i.e., `addAndFilter()` cannot follow `addAllChildren()` or
   *   `addAllChildrenWithFilter()`.
   *
   ...
   *
   * @param {AerospikExp} exp - filter expression to apply
   * @return {CdtContext} Updated CDT context, so calls can be chained.
   */
  addAndFilter (exp) {
    return this.add(AND_EXP, exp)
  }
