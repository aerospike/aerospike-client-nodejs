...
function _listJoin (bin, ctx) {
  return [
    { op: exp.ops.CALL, count: 5 },
    ..._rtype(exp.type.STR),
    ..._int(exp.sys.CALL_CDT),
    ..._listStart(ctx, lists.opcodes.STRING_LIST_JOIN, 0),
    ...bin
  ]
}

function _listJoinSeparator (bin, separator, ctx) {
  return [
    { op: exp.ops.CALL, count: 5 },
    ..._rtype(exp.type.STR),
    ..._int(exp.sys.CALL_CDT),
    ..._listStart(ctx, lists.opcodes.STRING_LIST_JOIN, 1),
 ...
 * Inverse of {@link module:aerospike/exp/string.splitSeparator}. Requires server 8.1.3+.
 * Pass a string separator as the second argument to join with a delimiter
 * (same as {@link module:aerospike/exp/lists.joinSeparator}).
 *
 * @param {AerospikeExp} bin List bin or list value expression.
 * @param {string|Object} [separatorOrCtx] Separator string, or optional context path for nested CDT.
 * @param {Object} [ctx] Optional context path when a separator is provided.
 * @return {AerospikeExp} (string expression)
 */
  join: (bin, separatorOrCtx = null, ctx = null) => (
    typeof separatorOrCtx === 'string'
      ? _listJoinSeparator(bin, separatorOrCtx, ctx)
      : _listJoin(bin, separatorOrCtx)
  ),

  /**
 * Concatenate the string items of a list with a separator and return a single string.
 * Same as {@link module:aerospike/exp/lists.join} with a separator. Requires server 8.1.3+.
 *
 * @param {AerospikeExp} bin List bin or list value expression.
 ...
 * @return {AerospikeExp} (string expression)
 */
  joinSeparator: (bin, separator, ctx = null) => _listJoinSeparator(bin, separator, ctx),
