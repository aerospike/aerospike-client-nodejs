 ...
 * @property {number} FLOAT - Floating-point only. Requires `.` plus a digit
 * (not `strtod`): `"3.14"` matches, `"5"` / `"5."` / `"1e5"` do not.
 */
 ...
 * @description Requires an applicable string bin; errors if the bin is missing or not a string.
 * {@link numericType.FLOAT} matches a decimal point followed by a digit, not
 * `strtod` (so `"3.14"` is true and `"5"` / `"5."` / `"1e5"` are false).
 * @param {string} bin - Bin name.
