 ...
 * @classdesc String bin operation for {@link Client#operate} (server 8.2.0+).
 */
class StringOperation extends Operation {}

/**
 * @summary Target a nested string via context instead of the top-level bin value.
 * @description Nested string targets use the same CDT context packing as list/map
 * ops: `[0xFF, ctx_list, [sub_op, args...]]`. An empty context omits the wrapper.
 * {@link module:aerospike/strings.toString|toString} is a top-level conversion
 * and does not apply context.
 *
 ...
 * Applies to string modify operations that support policy on the wire, including
 * {@link module:aerospike/strings.regexReplace|regexReplace}. Regex flags remain
 * a separate argument on that operation.
 *
 ...
 * @description Requires Aerospike Server 8.2.0 or later. See also
 ...
 * {@link module:aerospike/strings.writeFlags.CREATE_ONLY|CREATE_ONLY} (value 1)
 * is valid only on the eight create ops: {@link insert}, {@link overwrite},
 * {@link concat}, {@link append}, {@link prepend}, {@link padStart},
 * {@link padEnd}, and {@link repeat}. It is mutually exclusive with
 * {@link module:aerospike/strings.writeFlags.UPDATE_ONLY|UPDATE_ONLY} and
 * invalid with {@link module:aerospike/strings~StringOperation#withContext|withContext}.
 ...
 * suppress wrong-type or invalid-bin-type errors, and it does not suppress
 * parse errors such as combining CREATE_ONLY with UPDATE_ONLY.
 *
 * For richer failure data on server 8.2.0+, set
 ...
 *
 * @remarks Some server 8.2.0 builds may ignore {@link writeFlags.NO_FAIL} on
 ...
 *
 * @type Object
 * @property {number} DEFAULT - Default. Allow create or update.
 * @property {number} CREATE_ONLY - Create the bin only when it is missing.
 * Valid on insert, overwrite, concat, append, prepend, padStart, padEnd, and
 * repeat. Returns BIN_EXISTS if the bin already exists. Mutually exclusive
 * with {@link UPDATE_ONLY}. Invalid with a CDT context path.
 * @property {number} UPDATE_ONLY - Update existing values only. Mutually
 * exclusive with {@link CREATE_ONLY}.
 * @property {number} NO_FAIL - Suppress an operation failure with the bin
 * unchanged. Does not suppress wrong-type or invalid-bin-type errors, and does
 * not suppress parse errors.
 *
 ...
 *
 * @type Object
 * @property {number} ANY - Integer or floating-point. Integers match; floats
 * must include a decimal point and a digit (`3.14` yes, `5` / `5.` / `1e5` no).
 * @property {number} INT - Integer only.
 * @property {number} FLOAT - Floating-point only. Requires a decimal point
 * and a digit after it: `"3.14"` matches; `"5"`, `"5."`, and `"1e5"` do not.
 */
 ...
 * @description Matching is Unicode canonical (NFC vs NFD), not byte-exact.
 * Requires an applicable string bin; errors if the bin is missing or not a string.
 * @param {string} bin - Bin name.
 ...
 * @description Requires an applicable string bin; errors if the bin is missing or not a string.
 * Failed conversion returns {@link module:aerospike/status.ERR_OP_NOT_APPLICABLE|ERR_OP_NOT_APPLICABLE}
 * (26) with {@link module:aerospike/subcode.OPNOT_STRING_CONVERSION_FAILED|OPNOT_STRING_CONVERSION_FAILED}
 * (10).
 * @param {string} bin - Bin name.
 * @returns {module:aerospike/strings~StringOperation} Operation for {@link Client#operate}.
 */
 ...
 * @description Requires an applicable string bin; errors if the bin is missing or not a string.
 * {@link numericType.FLOAT} matches a decimal point followed by a digit
 * (`"3.14"` is true; `"5"`, `"5."`, and `"1e5"` are false).
 * @param {string} bin - Bin name.
 ...
 * @description Requires an applicable string bin; errors if the bin is missing or not a string.
 * Invalid base64 returns {@link module:aerospike/status.ERR_OP_NOT_APPLICABLE|ERR_OP_NOT_APPLICABLE}
 * (26) with {@link module:aerospike/subcode.OPNOT_STRING_B64_INVALID|OPNOT_STRING_B64_INVALID} (13).
 * @param {string} bin - Bin name.
 * @returns {module:aerospike/strings~StringOperation} Operation for {@link Client#operate}.
 */
...
/**
 * @summary Remove codepoints from `start` to `end`, or through the end of the string.
 * @description Packs `[53, start, end]` when `end` is given, or `[53, start]`
 * when `end` is omitted (same as {@link snipStart}). Policy flags are not sent
 * on the 1-arg form. If the bin is missing, no-op (record unchanged).
 * @param {string} bin - Bin name.
 * @param {number} start - First codepoint to remove, inclusive.
 * @param {number} [end] - One past the last codepoint to remove. Omit to remove through the end.
 * @returns {module:aerospike/strings~StringOperation} Operation for {@link Client#operate}.
 */
exports.snip = (bin, start, end) =>
  end === undefined ? op('SNIP_START', bin, { start }) : op('SNIP', bin, { start, end })

/**
 * @summary Remove from `start` through the end of the string.
 * @description Packs `[53, start]` only. Policy flags are not sent. If the bin
 * is missing, no-op (record unchanged).
 * @param {string} bin - Bin name.
 * @param {number} start - First codepoint to remove, inclusive.
 * @returns {module:aerospike/strings~StringOperation} Operation for {@link Client#operate}.
 */
exports.snipStart = (bin, start) => op('SNIP_START', bin, { start })
 ...
 * @description Matching is Unicode canonical (NFC vs NFD), not byte-exact.
 * If the bin is missing, no-op (record unchanged).
 * @param {string} bin - Bin name.
 * @param {string} needle - Substring to find.
 ...
 * @description Accepts {@link regexFlags} (e.g. GLOBAL for all matches) and
 * optional {@link writeFlags} via {@link StringOperation#withPolicy}. If the bin
 * is missing, no-op (record unchanged) unless policy flags change that behavior.
 * @param {string} bin - Bin name.
 * @param {string} pattern - Regex pattern.
 * @param {string} replacement - Replacement string.
 * @param {number} flags - {@link regexFlags} bit flags.
 ...
 * @description Accepted source types are integer, float, bool, string, and blob.
 * If the bin is missing, no-op (record unchanged). List, map, GeoJSON, and HLL
 * bins return {@link module:aerospike/status.ERR_BIN_INCOMPATIBLE_TYPE|ERR_BIN_INCOMPATIBLE_TYPE}
 * (12). A blob whose bytes are not valid UTF-8 returns
 * {@link module:aerospike/status.ERR_OP_NOT_APPLICABLE|ERR_OP_NOT_APPLICABLE} (26)
 * with {@link module:aerospike/subcode.OPNOT_STRING_UTF8_INVALID|OPNOT_STRING_UTF8_INVALID}
