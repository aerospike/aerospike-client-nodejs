...
/**
 * Remove from `start` through the end of the string. Packs `[53, start]` only
 * (C `as_exp_string_snip_start`). Policy is accepted for API parity and is not
 * packed.
 *
 * @param {object|null} policy
 * @param {number|import('./exp').AerospikeExp} start
...
/**
 * Matches `as_exp_string_regex_replace` in as_exp.h (CLIENT-4824): quoted pair,
 * regex flags uint, then string policy flags uint.
 *
 * @param {object|null} policy `{ flags }` using {@link module:aerospike/strings~strings.writeFlags}
