...
//
// CTX: operate-path nested strings use CDT packing via the C client
// (`[0xFF, ctx_list, [sub_op, args...]]`). Expression-path StringExp does not
// take CTX; extract the leaf with list/map get expressions first (same as C
// `as_exp.h` and the Python helpers).
//
 ...
 * @summary String read/modify expressions (Aerospike Server 8.2.0+)
 *
 * @description String read and modify expressions. For `operate()` string bin
 * helpers, see {@link module:aerospike/strings|aerospike/strings}.
 *
 ...
 * `[start, end)` in Unicode codepoints.
 *
...
/** Matching is Unicode canonical (NFC vs NFD), not byte-exact. @param {string} prefix @param {import('./exp').AerospikeExp} bin */
...
/** Matching is Unicode canonical (NFC vs NFD), not byte-exact. @param {string} suffix @param {import('./exp').AerospikeExp} bin */
...
/**
 * Parse a string expression as an integer.
 * Failed conversion yields {@link module:aerospike/status.ERR_OP_NOT_APPLICABLE|ERR_OP_NOT_APPLICABLE}
...
/**
 * Parse a string expression as a double.
 * Failed conversion yields {@link module:aerospike/status.ERR_OP_NOT_APPLICABLE|ERR_OP_NOT_APPLICABLE}
 * (26) with {@link module:aerospike/subcode.OPNOT_STRING_CONVERSION_FAILED|OPNOT_STRING_CONVERSION_FAILED}
 * (10).
 * @param {import('./exp').AerospikeExp} bin
...
/**
 * FLOAT requires a decimal point and a digit after it: `"3.14"` true; `"5"` / `"5."` / `"1e5"` false.
 * @param {number} numericType {@link module:aerospike/strings~strings.numericType}
 * @param {import('./exp').AerospikeExp} bin
 */
...
/**
 * Base64-decode a string expression to a blob.
 * Invalid base64 yields {@link module:aerospike/status.ERR_OP_NOT_APPLICABLE|ERR_OP_NOT_APPLICABLE}
...
/**
 * Concatenate a list of string expressions onto the bin. The second argument
 * is an expression that evaluates to a list of strings
 * (e.g. {@link module:aerospike/exp#list|exp.list}).
 *
...
/**
 * Concatenate a literal string onto the bin. For a list-valued operand use
 * {@link #concatList}.
 *
 * @param {object|null} policy
 * @param {string} value
...
/**
 * Append a single string. Prefer this over {@link #concat} when appending one
 * literal; `concat` is for list-valued payloads.
 *
 * @param {object|null} policy
 * @param {string} value
 * @param {import('./exp').AerospikeExp} bin
...
/**
 * Remove half-open codepoint range `[start, end)`.
 *
 * @param {object|null} policy
 * @param {number|import('./exp').AerospikeExp} start
...
/**
 * Remove from `start` through the end of the string. Policy is accepted for API
 * parity with the other write expressions and is not packed.
 *
 * @param {object|null} policy
 * @param {number|import('./exp').AerospikeExp} start
 * @param {import('./exp').AerospikeExp} bin
 */
const snipStart = (_policy, start, bin) => [
  ..._stringModStart(OP.SNIP, 1),
  ...intOrExp(start),
  ...bin
]
...
/**
 * Regex replace. Wire order is quoted pair, regex flags, then string write
 * policy flags.
 *
 * @param {object|null} policy `{ flags }` using {@link module:aerospike/strings~strings.writeFlags}
 * @param {string} pattern
 * @param {string} replacement
 * @param {number} flags regex flags ({@link module:aerospike/strings~strings.regexFlags})
 * @param {import('./exp').AerospikeExp} bin
 */
const regexReplace = (policy, pattern, replacement, flags, bin) => [
  ..._stringModStart(OP.REGEX_REPLACE, 3),
  ..._quotedPair(
    [{ op: exp.ops.VAL_STR, strVal: pattern }],
    [{ op: exp.ops.VAL_STR, strVal: replacement }]
  ),
 ...
 * {@link module:aerospike/status.ERR_BIN_INCOMPATIBLE_TYPE|ERR_BIN_INCOMPATIBLE_TYPE}
