...
const _int = (n) => [{ op: exp.ops.VAL_INT, intVal: n }]
const _uint = (n) => [{ op: exp.ops.VAL_UINT, uintVal: n }]
const _rtype = (t) => [{ op: exp.ops.VAL_RTYPE, intVal: t }]
...
/** @param {number|import('./exp').AerospikeExp} start @param {import('./exp').AerospikeExp} bin */
const substr = (start, bin) => [
  ..._stringReadStart(exp.type.STR, OP.SUBSTR, 1),
  ...(typeof start === 'number' ? _int(start) : start),
  ...bin
]

/** @param {number|import('./exp').AerospikeExp} start @param {number|import('./exp').AerospikeExp} end @param {import('./exp').AerospikeExp} bin */
const substrRange = (start, end, bin) => [
  ..._stringReadStart(exp.type.STR, OP.SUBSTR, 2),
  ...(typeof start === 'number' ? _int(start) : start),
  ...(typeof end === 'number' ? _int(end) : end),
  ...bin
]
...
const charAt = (index, bin) => [
  ...
  ...(typeof index === 'number' ? _int(index) : index),
  ...bin
]
...
const findOccurrence = (needle, occurrence, bin) => [
  ...
  { op: exp.ops.VAL_STR, strVal: needle },
  ...(typeof occurrence === 'number' ? _int(occurrence) : occurrence),
  ...bin
]
...
/** @param {object|null} policy @param {import('./exp').AerospikeExp} values @param {import('./exp').AerospikeExp} bin */
...
const snip = (policy, start, bin) => [
  ..._stringModStart(OP.SNIP, 2),
  ...(typeof start === 'number' ? _int(start) : start),
  ..._policyFlags(policy),
  ...bin
]

const snipRange = (policy, start, end, bin) => [
...
const padStart = (policy, targetLength, padString, bin) => [
  ...
  ...(typeof targetLength === 'number' ? _int(targetLength) : targetLength),
...
const repeat = (policy, count, bin) => [
  ...
  ...(typeof count === 'number' ? _int(count) : count),
  ..._policyFlags(policy),
  ...bin
]

const regexReplace = (pattern, replacement, flags, bin) => [
