...
Client.prototype.infoAny = function (request, policy, callback) {
  ...
  if (typeof policy === 'object' && policy !== null) {
    // console.log(policy.constructor.name)
    if (!policy.constructor.name.includes('Policy')) {
      req = policy
      pol = null
    }
  }
  ...
  if (typeof request === 'object' && request !== null) {
    ...
    if (request.constructor.name.includes('Policy')) {
      pol = request
    }
  }

  const cmd = new Commands.InfoAny(this, [req, pol], callback)
  return cmd.execute()
}
...
Client.prototype.operate = function (key, operations, metadata, policy, callback) {
  ...
  if (typeof metadata === 'object' && metadata !== null) {
    ...
    if (metadata.constructor.name.includes('Policy')) {
      pol = metadata
    }
  }

  const cmd = new Commands.Operate(this, key, [operations, met, pol], callback)
  return cmd.execute()
}
...
Client.prototype.put = function (key, bins, meta, policy, callback) {
  ...
  if (typeof meta === 'object' && meta !== null) {
    // console.log(meta.constructor.name)
    if (meta.constructor.name.includes('Policy')) {
      pol = meta
    }
  }

  const cmd = new Commands.Put(this, key, [bins, met, pol], callback)
  return cmd.execute()
}
