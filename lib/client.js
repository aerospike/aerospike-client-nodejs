...
const IndexTask = require('./index_task.js')
 ...
 * the {@link IndexTask} instance returned by the callback.
 *
 ...
 *
 *   client.createIndex(options, policy, (error, indexTask) => {
 *     if (error) throw error
 *
...
Client.prototype.createIndex = function (options, policy, callback) {
  ...
  this.as_client.indexCreate(
    ...
    function createIndexCb (err) {
      var task = new IndexTask(self, options.ns, options.index)
      self.callbackHandler(callback, err, task)
    }
  )
}
...
Client.prototype.get = function (key, policy, callback) {
  ...
  var self = this
  this.as_client.getAsync(key, policy, function (err, record, metadata) {
 ...
 * IndexTask#waitUntilDone} method on the {@link IndexTask} instance returned
 ...
 *
 * @deprecated since v2.0 - use {@link IndexTask#waitUntilDone} instead.
 */
Client.prototype.indexCreateWait = function (namespace, index, pollInterval, callback) {
  ...
  var indexTask = new IndexTask(this, namespace, index)
  indexTask.waitUntilDone(pollInterval, callback)
}
...
Client.prototype.info = function (request, host, policy, infoCb, doneCb) {
  ...
  var self = this
  this.as_client.info(request, host, policy, function (err, response, host) {
 ...
 * @param {Predicate[]} [statement.filters] - Filter predicates to apply to the query. See {@link module:aerospike.filter}.
 * @param {string[]} [statement.select] - List of bin names to select.
 * @param {object} [statement.aggregationUDF] - Parameters for Stream UDF to
 * apply to the query in order to aggregate the results.
 * @param {string} statement.aggregationUDF.module - Lua user defined function module name.
 * @param {string} statement.aggregationUDF.funcname - Lua user defined function name.
 * @param {Array<*>} [statement.aggregationUDF.args] - Lua user defined function arguments.
 *
 * @see {@link Query}
 *
 * @example
 ...
 * @param {object} [options] - Scan parameters. See {@link Scan} constructor.
 *
 * @since v2.0
 */
...
/**
 * @callback Client~taskCallback
 *
 * @summary Function called when a potentially long-running task has been started.
 *
 * @param {?AerospikeError} error - The error code and message or <code>null</code> if the operation was successful.
 * @param {(IndexTask|ScanTask)} [task] - Handle on a potentially long-running task which can be used to check for task completion.
 */
