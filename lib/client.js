...
Client.prototype.infoAll = function (request, policy, callback) {
  ...
  if (!this.isConnected(false)) {
    let error = new AerospikeError(as.status.AEROSPIKE_ERR_CLIENT, 'Not connected.')
    if (callback) {
...
Client.prototype.put = function (key, record, metadata, policy, callback) {
  ...
  let cmd = new WriteRecordCommand(this, 'putAsync', [key, record, metadata, policy], callback)
  return cmd.execute()
}
