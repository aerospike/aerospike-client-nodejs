...
function convertBatchResults (results) {
  if (results) {
 ...
 * @returns {?Promise} - If no callback function is passed, the function returns a Promise that resolves to the results of the batch opertion.
 *
...
Client.prototype.batchExists = function (keys, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('batchExists', [keys, policy],
      (err, results) => callback(err, convertBatchResults(results)))
  } else {
    return this.sendCommandAsync('batchExists', [keys, policy], convertBatchResults)
  }
}
...
Client.prototype.batchGet = function (keys, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('batchGet', [keys, policy],
      (err, results) => callback(err, convertBatchResults(results)))
  } else {
    return this.sendCommandAsync('batchGet', [keys, policy], convertBatchResults)
  }
}
...
Client.prototype.batchRead = function (records, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('batchRead', [records, policy],
      (err, results) => callback(err, convertBatchResults(results)))
  } else {
    return this.sendCommandAsync('batchRead', [records, policy], convertBatchResults)
  }
}
...
Client.prototype.batchSelect = function (keys, bins, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('batchSelect', [keys, bins, policy],
      (err, results) => callback(err, convertBatchResults(results)))
  } else {
    return this.sendCommandAsync('batchSelect', [keys, bins, policy], convertBatchResults)
  }
}
...
Client.prototype.createIndex = function (options, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('indexCreate', args, error => {
      if (error) {
        callback(error)
      } else {
        callback(null, new IndexJob(this, options.ns, options.index))
      }
    })
  } else {
    return this.sendCommandAsync('indexCreate', args,
      () => new IndexJob(this, options.ns, options.index))
  }
}
 ...
 * @param {Client~valueCallback} callback - The function to call when the operation has completed with the results of the operation.
 *
 * @since v2.0
 *
...
Client.prototype.apply = function (key, udfArgs, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('applyAsync', [key, udfArgs, policy], callback)
  } else {
    return this.sendCommandAsync('applyAsync', [key, udfArgs, policy])
  }
}
...
Client.prototype.exists = function exists (key, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('existsAsync', [key, policy], error => {
    ...
    return this.sendCommandAsync('existsAsync', [key, policy])
      .then(() => Promise.resolve(true))
      .catch(error => {
        if (error.code === as.status.AEROSPIKE_ERR_RECORD_NOT_FOUND) {
...
Client.prototype.get = function (key, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('getAsync', [key, policy],
      (err, bins, metadata) => callback(err, new Record(key, bins, metadata)))
  } else {
    return this.sendCommandAsync('getAsync', [key, policy],
      (bins, metadata) => new Record(key, bins, metadata))
  }
}
...
Client.prototype.indexRemove = function (namespace, index, policy, callback) {
  ...
  if (typeof callback === 'function') {
    ...
    return this.sendCommandAsync('indexRemove', [namespace, index, policy])
  }
}
...
Client.prototype.info = function (request, host, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('info', [request, host, policy], callback)
  } else {
    return this.sendCommandAsync('info', [request, host, policy])
  }
}
...
Client.prototype.infoAny = function (request, policy, callback) {
  ...
  if (typeof callback === 'function') {
    ...
    return this.sendCommandAsync('info', [request, null, policy])
  }
}
...
Client.prototype.infoAll = function (request, policy, callback) {
  ...
  if (!this.isConnected(false)) {
    return this.sendError(callback, 'Not connected.')
  }

  let error = null
  let info = []
  let infoCb = (err, response, host) => {
    err = AerospikeError.fromASError(err)
    error = error || err
  ...
  if (typeof callback === 'function') {
    this.as_client.infoForeach(request, policy, infoCb, () => callback(error, info))
  } else {
    return new Promise((resolve, reject) => {
      this.as_client.infoForeach(request, policy, infoCb, () => {
        if (error) {
          reject(error)
        } else {
          resolve(info)
...
Client.prototype.operate = function (key, operations, metadata, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('operateAsync', [key, operations, metadata, policy],
      (err, bins, metadata) => callback(err, new Record(key, bins, metadata)))
  } else {
    return this.sendCommandAsync('operateAsync', [key, operations, metadata, policy],
      (bins, metadata) => new Record(key, bins, metadata))
  }
}
...
Client.prototype.put = function (key, record, metadata, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('putAsync', [key, record, metadata, policy],
      (err) => callback(err, key))
  } else {
    return this.sendCommandAsync('putAsync', [key, record, metadata, policy],
      () => key)
  }
}
...
Client.prototype.remove = function (key, policy, callback) {
  ...
  let args = [key, policy]
  if (typeof callback === 'function') {
    this.sendCommand(cmd, args, (err) => callback(err, key))
  } else {
    return this.sendCommandAsync(cmd, args, () => key)
  }
}
...
Client.prototype.select = function (key, bins, policy, callback) {
  ...
  let cmd = 'selectAsync'
  let args = [key, bins, policy]
  if (typeof callback === 'function') {
    this.sendCommand(cmd, args, (err, bins, metadata) => {
      callback(err, new Record(key, bins, metadata))
    })
  } else {
    return this.sendCommandAsync(cmd, args, (bins, metadata) =>
      new Record(key, bins, metadata))
  }
}
...
Client.prototype.sendCommand = function (cmd, args, callback) {
  ...
  if (typeof callback !== 'function') {
    throw new AerospikeError('Missing callback function argument')
  }

  let stackTrace = null
  if (this.captureStackTraces) {
    stackTrace = AerospikeError.captureStackTrace()
  }
  ...
  args.push(function (err, arg1, arg2, arg3) {
    err = AerospikeError.fromASError(err, stackTrace)
    if (sync) {
      process.nextTick(callback, err, arg1, arg2, arg3)
    } else {
      return callback(err, arg1, arg2, arg3)
    }
  })
 ...
 * @return { Promise }
 **/
Client.prototype.sendCommandAsync = function (cmd, args, builder) {
  if (!this.isConnected(false)) {
    return this.sendError(null, 'Not connected.')
  }
  ...
  return new Promise((resolve, reject) => {
    args.push(function (err, arg1, arg2, arg3) {
      ...
      if (err) {
        ...
        resolve(builder(arg1, arg2, arg3))
      } else {
        resolve(arg1)
      }
    })
...
/** @private */
Client.prototype.sendError = function (callback, message) {
  let error = new AerospikeError(as.status.AEROSPIKE_ERR_CLIENT, message)
  if (callback) {
...
Client.prototype.udfRegister = function (udfPath, udfType, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('udfRegister', [udfPath, udfType, policy], error => {
      if (error) {
        callback(error)
      } else {
        callback(null, new UdfJob(this, module, UdfJob.REGISTER))
      }
    })
  } else {
    return this.sendCommandAsync('udfRegister', [udfPath, udfType, policy], () =>
      new UdfJob(this, module, UdfJob.REGISTER))
  }
}
...
Client.prototype.udfRemove = function (udfModule, policy, callback) {
  ...
  if (typeof callback === 'function') {
    this.sendCommand('udfRemove', [udfModule, policy], error => {
      if (error) {
        callback(error)
      } else {
        callback(null, new UdfJob(this, udfModule, UdfJob.UNREGISTER))
      }
    })
  } else {
    return this.sendCommandAsync('udfRemove', [udfModule, policy], () =>
      new UdfJob(this, udfModule, UdfJob.UNREGISTER))
  }
}
