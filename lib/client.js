...
// CLIENT-4358: The Aerospike C client already supports AS_INDEX_INTEGER
// and maps INTEGER <-> NUMERIC by server version (8.1.3 boundary) in
// aerospike_index_create_private(). Node passes the datatype through to the native
// binding; no version-specific mapping is done in this layer.
const DEPRECATED_NUMERIC_INDEX_WARNING =
  'Aerospike.indexDataType.NUMERIC is deprecated; use Aerospike.indexDataType.INTEGER instead.'

function warnDeprecatedNumericIndexDataType () {
  process.emitWarning(
    DEPRECATED_NUMERIC_INDEX_WARNING,
    'DeprecationWarning',
    'AEROSPIKE_INDEX_DATATYPE_NUMERIC'
  )
}

function prepareIndexDataType (datatype) {
  if (datatype === as.indexDataType.NUMERIC) {
    warnDeprecatedNumericIndexDataType()
  }
  return datatype
}
...
function Client (config) {
  ...
  if (this.as_client === null) {
    throw new AerospikeError('Invalid client configuration')
  }
...
Client.prototype.expressionToBase64 = function (expression) {
  return this.as_client.expressionToBase64(expression)
}
 ...
 * })()
 */
Client.prototype.changePassword = function (user, password, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.ChangePassword(this, [user, password, policy, this.config.user], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.createUser = function (user, password, roles, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.UserCreate(this, [user, password, roles, policy], callback)
  return cmd.execute()
}

Client.prototype.createPKIUser = function (user, roles, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.UserCreatePKI(this, [user, roles, policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.createRole = function (roleName, privileges, policy, whitelist, readQuota, writeQuota, callback) {
  const cmd = new Commands.RoleCreate(this, [roleName, privileges, policy, whitelist, readQuota, writeQuota], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.dropRole = function (roleName, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.RoleDrop(this, [roleName, policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.dropUser = function (user, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.UserDrop(this, [user, policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.grantPrivileges = function (roleName, privileges, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.PrivilegeGrant(this, [roleName, privileges, policy], callback)
  return cmd.execute()
}
 ...
 *
 */
Client.prototype.grantRoles = function (user, roles, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.RoleGrant(this, [user, roles, policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.queryRole = async function (roleName, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.QueryRole(this, [roleName, policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.queryRoles = function (policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.QueryRoles(this, [policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.queryUser = function (user, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.QueryUser(this, [user, policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.queryUsers = function (policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.QueryUsers(this, [policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.revokePrivileges = function (roleName, privileges, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.PrivilegeRevoke(this, [roleName, privileges, policy], callback)
  return cmd.execute()
}
 ...
 * })()
 */
Client.prototype.revokeRoles = function (user, roles, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.RoleRevoke(this, [user, roles, policy], callback)
  return cmd.execute()
}

Client.prototype.setPassword = function (user, password, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.SetPassword(this, [user, password, policy, this.config.user], callback)
  return cmd.execute()
}
 ...
 *
 */
Client.prototype.setQuotas = function (roleName, readQuota, writeQuota, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.RoleSetQuotas(this, [roleName, readQuota, writeQuota, policy], callback)
  return cmd.execute()
}
 ...
 *
 */
Client.prototype.setWhitelist = function (roleName, whitelist, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }

  const cmd = new Commands.RoleSetWhitelist(this, [roleName, whitelist, policy], callback)
  return cmd.execute()
}
...
Client.prototype.createIndex = function (options, policy, callback) {
  ...
  const datatype = prepareIndexDataType(options.datatype)
...
Client.prototype.createExpIndex = function (options, policy, callback) {
  if (typeof policy === 'function') {
    callback = policy
    policy = null
  }
...
Client.prototype.createIntegerIndex = function (options, policy, callback) {
  options.datatype = as.indexDataType.INTEGER
  return this.createIndex(options, policy, callback)
}

Client.prototype.createExpIntegerIndex = function (options, policy, callback) {
  options.datatype = as.indexDataType.INTEGER
  return this.createExpIndex(options, policy, callback)
}
...
Client.prototype.createExpStringIndex = function (options, policy, callback) {
  options.datatype = as.indexDataType.STRING
  return this.createExpIndex(options, policy, callback)
}
...
Client.prototype.createExpGeo2DSphereIndex = function (options, policy, callback) {
  options.datatype = as.indexDataType.GEO2DSPHERE
  return this.createExpIndex(options, policy, callback)
}
/**
...
Client.prototype.createExpBlobIndex = function (options, policy, callback) {
  options.datatype = as.indexDataType.BLOB
  return this.createExpIndex(options, policy, callback)
}
