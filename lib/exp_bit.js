 ...
 */
  ...
  /**
 * Return the base64 text of a blob as a string. Requires server 8.1.3+.
 * Omit byteOffset to encode the whole blob; pass only byteOffset to encode
 * from that offset through the end; pass byteOffset and byteSize for a span.
 *
 * @param {AerospikeExp} bin A blob bin expression to apply this function to.
 * @param {AerospikeExp} [byteOffset] Byte offset into the blob. Negative values count from the end.
 * @param {AerospikeExp} [byteSize] Number of bytes to encode. Omit to encode through the end.
 * @param {boolean} [invertSize] When true, count byteSize back from the blob end.
 * @return {AerospikeExp} string value
 */
  b64Encode: (bin, byteOffset, byteSize, invertSize = false) => {
    if (byteOffset === undefined) {
      return [
        ..._bitReadStart(exp.type.STR, bits.opcodes.B64_ENCODE, 0),
        ...bin
      ]
    }
    if (byteSize === undefined) {
      return [
        ..._bitReadStart(exp.type.STR, bits.opcodes.B64_ENCODE, 1),
        ...byteOffset,
        ...bin
      ]
    }
    return [
      ..._bitReadStart(exp.type.STR, bits.opcodes.B64_ENCODE, 3),
      ...byteOffset,
      ...byteSize,
      ..._int(invertSize ? 1 : 0),
      ...bin
  ...
  /**
 * Return the base64 text from byteOffset through the end of the blob as a string.
 *
 * @param {AerospikeExp} bin A blob bin expression to apply this function to.
 * @param {AerospikeExp} byteOffset Byte offset into the blob. Negative values count from the end.
 * @return {AerospikeExp} string value
 */
  b64EncodeFrom: (bin, byteOffset) => [
    ..._bitReadStart(exp.type.STR, bits.opcodes.B64_ENCODE, 1),
    ...byteOffset,
    ...bin
  ],
  ...
  /**
 * Return the base64 text of a byte range of the blob as a string.
 * When invertSize is true, byteSize counts back from the blob end, so 0 means to the end.
 *
 * @param {AerospikeExp} bin A blob bin expression to apply this function to.
 * @param {AerospikeExp} byteOffset Byte offset into the blob.
 * @param {AerospikeExp} byteSize Number of bytes to encode.
 * @param {boolean} invertSize When true, count byteSize back from the blob end.
 * @return {AerospikeExp} string value
 */
  b64EncodeRange: (bin, byteOffset, byteSize, invertSize = false) => [
    ..._bitReadStart(exp.type.STR, bits.opcodes.B64_ENCODE, 3),
    ...byteOffset,
    ...byteSize,
    ..._int(invertSize ? 1 : 0),
