...
/**
 * @summary Create bit "b64 encode" operation.
 * @description Omit `byteOffset` to encode the whole blob. Pass only
 * `byteOffset` to encode from that offset through the end. Pass `byteOffset`
 * and `byteSize` for a span. When `invertSize` is true, `byteSize` counts back
 * from the blob end rather than forward from `byteOffset`, so a `byteSize` of
 * 0 means to the end of the blob. There is no `-1` sentinel; use `invertSize`
 * for that case. Requires Aerospike Server 8.1.3 or later. Inverse of
 * {@link module:aerospike/strings.b64Decode}.
 *
 * @param {string} bin - The name of the bin. The bin must contain a byte value.
 * @param {number} [byteOffset] - Byte offset into the blob. Negative values count from the end.
 * @param {number} [byteSize] - Number of bytes to encode. Omit to encode through the end.
 * @param {boolean} [invertSize=false] - Count `byteSize` back from the blob end.
 * @returns {BitwiseOperation} Operation that can be passed to the {@link Client#operate} command.
 */
exports.b64Encode = function (bin, byteOffset, byteSize, invertSize) {
  if (byteOffset === undefined) {
    return new BitwiseOperation(opcodes.BIT_B64_ENCODE, bin)
  }
  if (byteSize === undefined) {
    return new BitwiseOperation(opcodes.BIT_B64_ENCODE_FROM, bin, { byteOffset })
  }
  return new BitwiseOperation(opcodes.BIT_B64_ENCODE_RANGE, bin, {
    byteOffset,
    byteSize,
    invertSize: invertSize === true
  })
}

/**
 * @summary Create bit "b64 encode" operation from a byte offset through the end of the blob.
 * @description Same as {@link module:aerospike/bitwise.b64Encode} with `byteOffset` only.
 * A negative `byteOffset` counts back from the end. Span is in bytes.
 * Requires Aerospike Server 8.1.3 or later.
 *
 * @param {string} bin - The name of the bin. The bin must contain a byte value.
 * @param {number} byteOffset - Byte offset into the blob.
 * @returns {BitwiseOperation} Operation that can be passed to the {@link Client#operate} command.
 */
exports.b64EncodeFrom = function (bin, byteOffset) {
  return exports.b64Encode(bin, byteOffset)
}

/**
 * @summary Create bit "b64 encode" operation on a byte span.
 * @description Same as {@link module:aerospike/bitwise.b64Encode} with offset,
 * size, and optional `invertSize`. When `invertSize` is true, `byteSize`
 * counts back from the blob end rather than forward from `byteOffset`, so a
 * `byteSize` of 0 means to the end of the blob. There is no `-1` sentinel; use
 * `invertSize` for that case. Requires Aerospike Server 8.1.3 or later.
 *
 * @param {string} bin - The name of the bin. The bin must contain a byte value.
 * @param {number} byteOffset - Byte offset into the blob. Negative values count from the end.
 * @param {number} byteSize - Number of bytes to encode.
 * @param {boolean} [invertSize=false] - Count `byteSize` back from the blob end.
 * @returns {BitwiseOperation} Operation that can be passed to the {@link Client#operate} command.
 */
exports.b64EncodeRange = function (bin, byteOffset, byteSize, invertSize = false) {
  return exports.b64Encode(bin, byteOffset, byteSize, invertSize)
}
