...
/**
 * @summary Create bit "b64 encode" operation for the whole blob.
 * @description Server returns the base64 text of the blob as a string.
 * Requires Aerospike Server 8.1.3 or later. Inverse of {@link module:aerospike/strings.b64Decode}.
 *
 * @param {string} bin - The name of the bin. The bin must contain a byte value.
 * @returns {BitwiseOperation} Operation that can be passed to the {@link Client#operate} command.
 */
exports.b64Encode = function (bin) {
 ...
 * @description A negative `byteOffset` counts back from the end. Span is in bytes.
 ...
 * @description When `invertSize` is true, `byteSize` counts back from the blob
 * end rather than forward from `byteOffset`, so a `byteSize` of 0 means to the
 * end of the blob. There is no `-1` sentinel; use `invertSize` for that case.
 * Requires Aerospike Server 8.1.3 or later.
 *
 * @param {string} bin - The name of the bin. The bin must contain a byte value.
