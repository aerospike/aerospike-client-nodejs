...
Query.prototype.execute = function () {
  ...
  var onError = function onError (error) {
    error = AerospikeError.fromASError(error)
    rs.emit('error', error)
  }
...
Query.prototype.info = function (scanId, callback) {
  this._query.queryInfo(scanId, function (scanInfo, scanID) {
    scanInfo.recordScanned = scanInfo.recordsScanned // for backwards compatibility
    callback(scanInfo, scanID)
  })
}
