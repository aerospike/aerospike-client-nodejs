...
const MAX_ORDER_BY_BIN_NAME_LENGTH = 15
const MIN_TOP_K = 1
 ...
 *
 * @description Top-K (`ORDER BY <bin> LIMIT k`) ranks and limits results
 * server-side. Because Aerospike has no schema, <code>type</code> must be
 * declared explicitly; it is not inferred from the bin's actual on-server
 * value.
 *
 * @param {string} binName - Name of the (projected or physical) bin to order by. Max. 15 characters.
...
Query.prototype.orderBy = function (binName, type, direction, flags) {
  ...
  if (binName.length > MAX_ORDER_BY_BIN_NAME_LENGTH) {
    throw paramError('orderBy bin name exceeds 15-character limit')
  }
 ...
 * @function Query#min
 *
 * @summary Finds the minimum value of a scalar bin across the query's result set.
 *
 * @param {string} binName - Name of the scalar bin to minimize. Max. 15 characters.
 * @param {number} type - One of {@link module:aerospike/query.orderByType}: INTEGER, DOUBLE, STRING, BYTES.
 * @param {QueryPolicy} [policy] - The Query Policy to use for this operation.
 *
 * @throws {AerospikeError} <code>ERR_PARAM</code> if any argument is invalid.
 *
 * @returns {Promise<*>} The minimum value found, or <code>null</code> if no record in the result set qualified.
 *
 * @since v6.next
 *
 * @example <caption>Find the lowest score across a set of records.</caption>
 *
 * const Aerospike = require('aerospike')
 *
 * Aerospike.connect().then(async client => {
 *   const query = client.query('test', 'demo')
 *   const min = await query.min('score', Aerospike.query.orderByType.DOUBLE)
 *   client.close()
 * })
 *
 * @see {@link Query#max}
 */
Query.prototype.min = function (binName, type, policy) {
  return queryMinMax(this, binName, type, queryEnums.order.ASC, policy)
}

/**
 * @function Query#max
 *
 * @summary Finds the maximum value of a scalar bin across the query's result set.
 *
 * @param {string} binName - Name of the scalar bin to maximize. Max. 15 characters.
 * @param {number} type - One of {@link module:aerospike/query.orderByType}: INTEGER, DOUBLE, STRING, BYTES.
 * @param {QueryPolicy} [policy] - The Query Policy to use for this operation.
 *
 * @throws {AerospikeError} <code>ERR_PARAM</code> if any argument is invalid.
 *
 * @returns {Promise<*>} The maximum value found, or <code>null</code> if no record in the result set qualified.
 *
 * @since v6.next
 *
 * @see {@link Query#min}
 */
Query.prototype.max = function (binName, type, policy) {
  return queryMinMax(this, binName, type, queryEnums.order.DESC, policy)
}
...
// Collects the set of bin names this query is known to project, via
// Query#select or Query#operate's read operations (this.ops). Returns
// null if the query doesn't restrict projection at all, in which case any
// orderBy bin name is implicitly valid. Doesn't detect bins renamed via
// Aerospike.exp.operations.read(...).
...
// Shared implementation for Query#min/Query#max.
async function queryMinMax (query, binName, type, direction, policy) {
  if (typeof binName !== 'string' || binName.length === 0) {
    throw paramError('bin name is required')
  }

  if (!collectProjectedBinNames(query)) {
    query.select(binName)
  }

  query.orderBy(binName, type, direction)
  query.topK(1)

  const results = await query.results(policy)
  return results.length > 0 ? results[0].bins[binName] : null
}

// All request-time (pre-flight) checks for Query#orderBy/Query#topK,
// called from Query#foreach before any command is dispatched.
function assertValidOrderBy (query) {
  ...
  if (query.maxRecords && query.maxRecords > 0) {
    throw paramError('orderBy/topK cannot be combined with maxRecords; Top-K always scans its full candidate set')
  }
