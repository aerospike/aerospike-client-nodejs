...
const queryEnums = require('./query_enums')
const TopKReduce = require('./topk_reduce')
...
const MAX_ORDER_BY_BIN_NAME_LENGTH = 14
const MIN_TOP_K = 1
...
function Query (client, ns, set, options) {
  ...
  /**
   * Top-K global order key, set by {@link Query#orderBy}. <code>null</code>
   * if {@link Query#orderBy} has not been called.
   *
   * @member {?Object} Query#orderByBin
   * @property {string} binName - Name of the (projected or physical) bin to order by.
   * @property {number} type - One of {@link module:aerospike/query.orderByType}.
   * @property {number} direction - One of {@link module:aerospike/query.order}.
   * @property {number} flags - One of {@link module:aerospike/query.orderByFlags}.
   *
   * @see Use {@link Query#orderBy} to set this member variable.
   */
  this.orderByBin = null

  /**
   * Top-K global limit, set by {@link Query#topK}. <code>null</code> if
   * {@link Query#topK} has not been called.
   *
   * @member {?number} Query#topKLimit
   *
   * @see Use {@link Query#topK} to set this member variable.
   */
  this.topKLimit = null
}
...
Query.prototype.whereWithExp = function (indexFilter, expression) {
  if (indexFilter instanceof filter.SindexFilterPredicate) {
    this.setSindexFilter(indexFilter)
    this.filters[0].exp = expression
  } else {
    throw new TypeError('indexFilter should be a SindexFilterPredicate')
  }
}

Query.prototype.whereWithIndexName = function (indexFilter, indexName) {
  if (indexFilter instanceof filter.SindexFilterPredicate) {
    this.setSindexFilter(indexFilter)
    this.filters[0].indexName = indexName
  } else {
    throw new TypeError('indexFilter should be a SindexFilterPredicate')
  }
}
 ...
 *
 * @summary Declares the Top-K global order key. See {@link Query#topK}.
 *
 * @description Top-K is implemented as a client-side reduce layered on
 * top of an ordinary query - it has no server/native counterpart yet, so
 * all validation happens here and in {@link Query#topK}, and the actual
 * ranking is performed client-side once the query completes (see {@link
 * module:aerospike/query~TopKReduce}). Because Aerospike has no schema,
 * <code>type</code> must be declared explicitly; it is not inferred from
 * the bin's actual on-server value.
 *
 * @param {string} binName - Name of the (projected or physical) bin to order by. Max. 14 characters.
 * @param {number} type - One of {@link module:aerospike/query.orderByType}: INTEGER, DOUBLE, STRING, BYTES.
 * @param {number} [direction=Aerospike.query.order.DESC] - One of {@link module:aerospike/query.order}: ASC or DESC.
 * @param {number} [flags=Aerospike.query.orderByFlags.NONE] - One of {@link module:aerospike/query.orderByFlags}, e.g. CASE_INSENSITIVE (STRING only).
 *
 * @throws {AerospikeError} <code>ERR_PARAM</code> if any argument is invalid.
 *
 * @since v6.next
 *
 * @example <caption>Return the 10 records with the lexicographically-largest 'name' bin.</caption>
 *
 * const Aerospike = require('aerospike')
 *
 * Aerospike.connect().then(async client => {
 *   const query = client.query('test', 'demo')
 *   query.orderBy('name', Aerospike.query.orderByType.STRING)
 *   query.topK(10)
 *   const results = await query.results()
 *   client.close()
 * })
 */
Query.prototype.orderBy = function (binName, type, direction, flags) {
  if (typeof binName !== 'string' || binName.length === 0) {
    throw paramError('orderBy bin name must be a non-empty string')
  }
  if (binName.length > MAX_ORDER_BY_BIN_NAME_LENGTH) {
    throw paramError('orderBy bin name exceeds 14-character limit')
  }

  if (type === undefined || type === null) {
    throw paramError('orderBy requires a type declaration (INTEGER, DOUBLE, STRING, or BYTES); Aerospike has no schema')
  }
  const validTypes = [
    queryEnums.orderByType.INTEGER,
    queryEnums.orderByType.DOUBLE,
    queryEnums.orderByType.STRING,
    queryEnums.orderByType.BYTES
  ]
  if (!validTypes.includes(type)) {
    throw paramError(`orderBy type must be one of INTEGER, DOUBLE, STRING, BYTES; got ${type}`)
  }

  if (direction === undefined || direction === null) {
    direction = queryEnums.order.DESC
  }
  if (direction !== queryEnums.order.ASC && direction !== queryEnums.order.DESC) {
    throw paramError('orderBy direction must be ASC or DESC')
  }

  if (flags === undefined || flags === null) {
    flags = queryEnums.orderByFlags.NONE
  }
  if (flags === queryEnums.orderByFlags.CASE_INSENSITIVE && type !== queryEnums.orderByType.STRING) {
    throw paramError('orderBy flag CASE_INSENSITIVE is only valid with type STRING')
  }

  this.orderByBin = { binName, type, direction, flags }
}

/**
 * @function Query#topK
 *
 * @summary Sets the global Top-K limit. Requires {@link Query#orderBy} to
 * have been called first.
 *
 * @param {number} k - Must be an integer in the inclusive range [1, 1000].
 *
 * @throws {AerospikeError} <code>ERR_PARAM</code> if {@link Query#orderBy}
 * has not been called yet, or if <code>k</code> is out of range.
 *
 * @since v6.next
 *
 * @see {@link Query#orderBy}
 */
Query.prototype.topK = function (k) {
  if (!this.orderByBin) {
    throw paramError('topK() requires orderBy() to be called first')
  }
  if (!Number.isInteger(k) || k < MIN_TOP_K || k > MAX_TOP_K) {
    throw paramError(`topK must be in [1, 1000]; got ${k}`)
  }
...
Query.prototype.foreach = function (policy, dataCb, errorCb, endCb) {
  assertValidOrderBy(this)

  const reduce = this.orderByBin
    ? new TopKReduce(
      this.orderByBin.binName,
      this.orderByBin.type,
      this.orderByBin.direction,
      this.orderByBin.flags,
      this.topKLimit)
    : null
  const stream = new RecordStream(this.client, reduce)
...
function assertValidQueryOptions (options) {
  ...
  if (options.select && options.ops) {
    process.emitWarning('Operations and bin names are mutually exclusive.')
  }
}
...
// Collects the set of bin names this query is known to project, i.e. via
// Query#select or Query#operate's read operations (this.ops). Returns
// null if the query doesn't restrict projection at all (all bins are
// returned), in which case any orderBy bin name is implicitly valid.
//
// Note: this only recognizes plain Operation objects with a `.bin`
// property (see lib/operations.js) - projections created via
// Aerospike.exp.operations.read(...) with a renamed result bin are not
// currently detected here, so orderBy against such a projected name is
// not rejected even though it isn't a physical bin either. This matches
// this step's scope (client-side validation for the common case); expanding
// detection to cover expression-read projections is tracked as an open
// item in docs/design/vector-phase-1-client-design.md §9.
function collectProjectedBinNames (query) {
  const names = []
  if (Array.isArray(query.selected)) names.push(...query.selected)
  if (Array.isArray(query.ops)) {
    for (const op of query.ops) {
      if (op && typeof op.bin === 'string') names.push(op.bin)
    }
  }
  return names.length > 0 ? names : null
}

// All request-time (pre-flight) checks for Query#orderBy/Query#topK,
// called from Query#foreach (and therefore Query#results) before any
// command is dispatched - mirrors how assertValidQueryOptions already
// gates this file today. See
// docs/design/vector-phase-1-client-design.md §4.3.2 for the full
// rationale behind each check; messages are taken verbatim from that
// section so error text stays consistent with the Java/Python/Node design
// docs.
function assertValidOrderBy (query) {
  if (!query.orderByBin) return

  if (query.topKLimit === null || query.topKLimit === undefined) {
    // Node-specific: the cross-language design doc doesn't call this out
    // explicitly (other clients always pair Statement#setOrderBy with
    // Statement#setTopK in their examples), but since this client's
    // TopKReduce needs a bound k to operate, calling orderBy() without a
    // matching topK() is rejected here rather than left undefined.
    throw paramError('orderBy() requires topK() to also be called; Top-K queries always specify a limit')
  }

  if (query.paginate) {
    // Node-specific: other clients don't share this pagination model.
    throw paramError('orderBy/topK cannot be combined with paginate; Top-K is single-shot in v1')
  }

  if (query.udf) {
    throw paramError('orderBy is incompatible with aggregate UDFs; use the UDF\'s own reduce stage')
  }

  const binName = query.orderByBin.binName
  const projected = collectProjectedBinNames(query)
  if (projected && !projected.includes(binName)) {
    throw paramError(`orderBy bin '${binName}' is not in projection; add it to setOperations or remove projection`)
  }
}

// Guards the background-query entry points (Query#apply, Query#background,
// Query#operate), which have no result stream to rank.
function assertNotOrderByQuery (query) {
  if (query.orderByBin) {
    throw paramError('orderBy/topK is only valid on foreground (SELECT-style) queries; background (UPDATE-style) queries do not produce a result stream')
  }
}
