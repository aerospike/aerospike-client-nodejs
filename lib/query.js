...
Query.prototype.foreach = function (policy, dataCb, errorCb, endCb) {
  ...
  var queryCb = function (error, bins, meta, key) {
    if (error) {
      ...
      stream.emit('data', new Record(key, bins, meta))
    }
...
Query.prototype.apply = function (udfModule, udfFunction, udfArgs, policy, callback) {
  ...
  if (typeof callback === 'function') {
    ...
    return this.client.sendCommandAsync('queryApply', [this.ns, this.set, this, policy])
  }
}
...
Query.prototype.background = function (udfModule, udfFunction, udfArgs, policy, queryID, callback) {
  ...
  if (typeof callback === 'function') {
    this.client.sendCommand('queryBackground', [this.ns, this.set, this, policy, queryID], error => {
      if (error) {
        callback(error)
      } else {
        let module = this.filters.length > 0 ? 'query' : 'scan'
        callback(null, new Job(this.client, queryID, module))
      }
    })
  } else {
    return this.client.sendCommandAsync('queryBackground', [this.ns, this.set, this, policy, queryID], () => {
      let module = this.filters.length > 0 ? 'query' : 'scan'
