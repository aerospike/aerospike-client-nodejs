 ...
 *
 * With a secondary index (SI), the following queries can be made:
 *
 ...
 *
 * Before a filter predicate can be applied, a secondary index (SI) needs to be
 * created on the bins which the predicate matches on. Using the Node.js
 * client, a secondary index (SI) can be created using {@link Client#createIndex}.
 *
 * Currently, only a single secondary index (SI) filter predicate is supported for
 ...
 *
 * Predicate filter expressions can be combined with a secondary index (SI) filter.
 *
...
function Query (client, ns, set, options) {
   ...
   * @see Use {@link module:aerospike/filter} to create a secondary index (SI)
   * filter.
   */
 ...
 *
 * @summary Applies a secondary index (SI) filter and/or a predicate expression to
 * the query.
 *
 * @description Use a secondary index (SI) filter, a predicate expression, or both,
 * to limit the results returned by the query. This method takes either a
 * secondary index (SI) filter created using the {@link
 ...
 *
 * @example <caption>Applying a secondary index (SI) filter to find all records
 ...
 * })
 *
 * @see {@link module:aerospike/filter} to create secondary index (SI) filters.
