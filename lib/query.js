...
Query.prototype.foreach = function (policy, dataCb, errorCb, endCb) {
  if (this.udf) return this.foreachAggregation(policy, dataCb, errorCb, endCb)
...
Query.prototype.foreachAggregation = function (policy, dataCb, errorCb, endCb) {
  var stream = new RecordStream(this.client)
  if (dataCb) stream.on('data', dataCb)
  if (errorCb) stream.on('error', errorCb)
  if (endCb) stream.on('end', endCb)
  this.client.as_client.queryForeach(this.ns, this.set, this, policy, function (error, record, meta, key) {
    if (error && error.code !== as.status.AEROSPIKE_OK) {
      stream.emit('error', error)
    } else if (record === null) {
      stream.emit('end')
