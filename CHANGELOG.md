...
* **Breaking Changes**
  ...
  * `exp.string.regexReplace` takes **`policy` first** for API symmetry with other string modify expressions; **regex flags** are passed in the **`flags`** argument and are packed on the wire (CLIENT-4955 / C client `stage`).
...
* **Improvements**
  ...
  * [CLIENT-5213] Platform-scoped native prebuilds ship as **`@aerospike/prebuild-{platform}-{arch}`** optional dependencies; the main `aerospike` package no longer bundles all prebuilds (Pattern B without post-install prune).
