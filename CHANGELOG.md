...
* **Improvements**
  ...
  * Enhanced error detail support (Aerospike Server 8.2.0+): added `BasePolicy.errorDetailVerbosity` to request server subcodes and messages, exposed `AerospikeError.subcode`, and added `Aerospike.errorDetailVerbosity` (verbosity levels) and `Aerospike.subcode` (server subcode constants) modules.
  ...
  * Bump `aerospike-client-c` to `stage` @ `8f997f9c` (7.6.2; includes 7.6.1 string ops / extended errors, CLIENT-5378, CLIENT-5454, AER-6957, CLIENT-5495 TLS conn-recover leak on cluster teardown).
