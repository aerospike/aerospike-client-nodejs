...
var orderMap = function (binName, order, ctx) {
...
var orderByKey = function (binName, ctx) { return orderMap(binName, maps.order.KEY_ORDERED, ctx); };
var orderByKeyValue = function (binName, ctx) { return orderMap(binName, maps.order.KEY_VALUE_ORDERED, ctx); };
describe('client.operate() - CDT Map operations', function () {
    helper.skipUnlessSupportsFeature(aerospike_1.default.features.CDT_MAP, this);
    ...
    describe('maps.create', function () {
        ...
        it('Creates a new map from a cdt context', function () {
            return initState()
                .then(createRecord({ map: { c: 1, b: 2, a: 3 } }))
                .then(orderByKey('map'))
                .then(operate(maps.create('map', maps.order.KEY_ORDERED).withContext(function (ctx) { return ctx.addMapKeyCreate('nested'); })))
    ...
    describe('maps.put', function () {
        ...
        context('update-only write mode', function () {
            var updateOnlyPolicy = new aerospike_1.default.MapPolicy({
        ...
        context('with update-only flag', function () {
            ...
            var policy = new aerospike_1.default.MapPolicy({
        ...
        context('create-only write mode', function () {
            var createOnlyPolicy = new aerospike_1.default.MapPolicy({
        ...
        context('with nested map context', function () {
            ...
            it('adds the item to the nested map', function () {
                return initState()
                    ...
                    .then(operate(maps.put('list', 'd', 99).withContext(function (ctx) { return ctx.addListIndex(0); })))
        ...
        context('with nested map context', function () {
            ...
            it('adds each item to the nested map', function () {
                return initState()
                    ...
                    .then(operate(maps.putItems('map', { c: 99, d: 100 }).withContext(function (ctx) { return ctx.addMapKey('nested'); })))
                    .then(assertResultEql({ map: 4 }))
        ...
        context('with nested map context', function () {
            ...
            it('increments the value of the entry and returns the final value', function () {
                return initState()
                    ...
                    .then(operate(maps.increment('list', 'b', 10).withContext(function (ctx) { return ctx.addListIndex(1); })))
        ...
        context('with nested map context', function () {
            ...
            it('decrements the value of the entry and returns the final value', function () {
                return initState()
                    ...
                    .then(operate(maps.decrement('list', 'b', 10).withContext(function (ctx) { return ctx.addListIndex(0); })))
        ...
        context('with nested map context', function () {
            ...
            it('removes all entries from the map', function () {
                return initState()
                    ...
                    .then(operate(maps
                    .clear('map')
                    .withContext(function (ctx) { return ctx.addMapKey('nested').addMapKey('deepNested'); })))
        ...
        context('with nested map context', function () {
            ...
            it('removes a map entry identified by key', function () {
                return initState()
                    ...
                    .then(operate(maps.removeByKey('list', 'b').withContext(function (ctx) { return ctx.addListIndex(-1); })))
    ...
    describe('maps.removeByKeyList', function () {
        it('removes map entries identified by one or more keys', function () {
            return initState()
                ...
                .then(assertResultSatisfy(function (result) { return eql(result.map.sort(), [1, 3]); }))
        ...
        context('with nested map context', function () {
            ...
            it('removes map entries identified by one or more keys', function () {
                return initState()
                    ...
                    .then(operate(maps.removeByKeyList('maps', ['a', 'c']).withContext(function (ctx) { return ctx.addMapKey('a'); })))
        ...
        context('with nested map context', function () {
            ...
            it('removes a map entry identified by value', function () {
                return initState()
                    ...
                    .withContext(function (ctx) { return ctx.addListValue({ a: 1, b: 2 }); }) // matches only the first list value
        ...
        context('with nested map context', function () {
            ...
            it('removes map entries identified by one or more values', function () {
                return initState()
                    ...
                    .then(operate(maps.removeByValueList('map', [1, 3]).withContext(function (ctx) { return ctx.addMapKey('a'); })))
        ...
        context('with nested map context', function () {
            ...
            it('removes map entries identified by value range', function () {
                return initState()
                    ...
                    .then(operate(maps.removeByValueRange('list', 2, 3).withContext(function (ctx) { return ctx.addListIndex(-1); })))
    ...
    describe('maps.removeByIndex', function () {
        ...
        context('with nested map context', function () {
            ...
            it('removes a map entry identified by index', function () {
                return initState()
                    .then(createRecord({ map: { nested: { a: 1, b: 2, c: 3 } } }))
                    .then(operate(maps.removeByIndex('map', 1).withContext(function (ctx) { return ctx.addMapValue({ a: 1, b: 2, c: 3 }); })))
        ...
        context('with nested map context', function () {
            ...
            it('removes map entries identified by index range', function () {
                return initState()
                    ...
                    .then(operate(maps.removeByIndexRange('map', 1, 2).withContext(function (ctx) { return ctx.addMapKey('nested'); })))
        ...
        context('with nested map context', function () {
            ...
            it('removes a map entry identified by rank', function () {
                return initState()
                    ...
                    .then(operate(maps.removeByRank('list', 0).withContext(function (ctx) { return ctx.addListIndex(0); })))
    ...
    describe('maps.removeByRankRange', function () {
        ...
        context('with nested map context', function () {
            ...
            it('removes map entries identified by rank range', function () {
                return initState()
                    .then(createRecord({ list: [{ a: 3, b: 2, c: 1 }] }))
                    .then(operate(maps.removeByRankRange('list', 0, 2).withContext(function (ctx) { return ctx.addListIndex(-1); })))
    ...
    describe('maps.size', function () {
        ...
        context('with nested map context', function () {
            ...
            it('returns the size of the map', function () {
                return initState()
                    .then(createRecord({ map: { nested: { a: 1, b: 2, c: 3 } } }))
                    .then(operate(maps.size('map').withContext(function (ctx) { return ctx.addMapKey('nested'); })))
                    .then(assertResultEql({ map: 3 }))
                    .then(cleanup());
            });
        });
        ...
        context('with nested map context', function () {
            ...
            it('fetches a map entry identified by value', function () {
                return initState()
                    ...
                    .then(operate(maps
                    .getByValue('map', 2)
                    .withContext(function (ctx) { return ctx.addMapKey('nested'); })
                    .andReturn(maps.returnType.KEY)))
                    .then(assertResultSatisfy(function (result) { return eql(result.map.sort(), ['b', 'c']); }))
                    .then(cleanup());
            });
        });
    });
    ...
    describe('maps.getByValueRange', function () {
        ...
        it('fetches all values from the specified start value until the end', function () {
            return initState()
                ...
                .then(assertResultSatisfy(function (result) { return eql(result.map.sort(), [2, 3]); }))
                .then(cleanup());
        });
        ...
        context('with nested map context', function () {
            ...
            it('fetches a map entry identified by index', function () {
                return initState()
                    ...
                    .then(operate(maps
                    .getByIndex('list', 1)
                    .withContext(function (ctx) { return ctx.addListIndex(4); })
                    .andReturn(maps.returnType.KEY_VALUE)))
                    .then(assertResultEql({ list: ['b', 2] }))
                    .then(cleanup());
            });
