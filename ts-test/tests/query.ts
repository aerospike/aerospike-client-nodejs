...
describe('Queries', function () {
  ...
  before(() => {
    ...
    if (helper.cluster.isVersionInRange('>= 6.1.0')) {
      ...
      samples.push({ name: 'nested point match', g: { nested: new GeoJSON.Point(103.913, 1.308) } })
      samples.push({ name: 'nested point non-match', g: { nested: new GeoJSON.Point(-122.101, 37.421) } })
      samples.push({ name: 'nested point list match', lg: { nested: [new GeoJSON.Point(103.913, 1.308), new GeoJSON.Point(105.913, 3.308)] } })
      samples.push({ name: 'nested point list non-match', lg: { nested: [new GeoJSON.Point(-122.101, 37.421), new GeoJSON.Point(-120.101, 39.421)] } })
      samples.push({ name: 'nested point map match', mg: { nested: { a: new GeoJSON.Point(103.913, 1.308), b: new GeoJSON.Point(105.913, 3.308) } } })
      samples.push({ name: 'nested point map non-match', mg: { nested: { a: new GeoJSON.Point(-122.101, 37.421), b: new GeoJSON.Point(-120.101, 39.421) } } })
      samples.push({ name: 'nested region match', g: { nested: new GeoJSON.Polygon([102.913, 0.308], [102.913, 2.308], [104.913, 2.308], [104.913, 0.308], [102.913, 0.308]) } })
      samples.push({ name: 'nested region non-match', g: { nested: new GeoJSON.Polygon([-121.101, 36.421], [-121.101, 38.421], [-123.101, 38.421], [-123.101, 36.421], [-121.101, 36.421]) } })
      samples.push({ name: 'nested region list match', lg: { nested: [new GeoJSON.Polygon([102.913, 0.308], [102.913, 2.308], [104.913, 2.308], [104.913, 0.308], [102.913, 0.308])] } })
      samples.push({ name: 'nested region list non-match', lg: { nested: [new GeoJSON.Polygon([-121.101, 36.421], [-121.101, 38.421], [-123.101, 38.421], [-123.101, 36.421], [-121.101, 36.421])] } })
      samples.push({ name: 'nested region map match', mg: { nested: { a: new GeoJSON.Polygon([102.913, 0.308], [102.913, 2.308], [104.913, 2.308], [104.913, 0.308], [102.913, 0.308]) } } })
      samples.push({ name: 'nested region map non-match', mg: { nested: [new GeoJSON.Polygon([-121.101, 36.421], [-121.101, 38.421], [-123.101, 38.421], [-123.101, 36.421], [-121.101, 36.421])] } })
    ...
    return Promise.all([
      putgen.put(numberOfSamples, generators)
        .then((records: AerospikeRecord[]) => { keys = records.map((rec: AerospikeRecord) => rec.key) })
        .then(() => Promise.all(indexes.map((idx: Array<any>) =>
    ...
    context('filter predicates', function () {
      describe('filter.equal()', function () {
        it('should match equal integer values', function (done) {
          const args: QueryOptions = { filters: [filter.equal('i', 5)] }
        ...
        context('Uses blob Secondary indexes', function () {
          ...
          it('should match equal blob values', function (done) {
            const args: QueryOptions = { filters: [filter.equal('blob', Buffer.from('guava'))] }
      ...
      describe('filter.range()', function () {
        it('should match integers within a range', function (done) {
          const args: QueryOptions = { filters: [filter.range('i', 3, 7)] }
          verifyQueryResults(args, 'int match', done)
        })

        it('should match integers in a list within a range', function (done) {
          const args: QueryOptions = { filters: [filter.range('li', 3, 7, LIST)] }
        ...
        describe('index with cdt context', function () {
          ...
          it('should match integers in a list within a range in a nested context', function (done) {
            const args: QueryOptions = { filters: [filter.range('li', 3, 7, LIST, new Context().addMapKey('nested'))] }
            verifyQueryResults(args, 'nested int list match', done)
          })
        })

        it('should match integers in a map within a range', function (done) {
          const args: QueryOptions = { filters: [filter.range('mi', 3, 7, MAPVALUES)] }
        ...
        describe('index with cdt context', function () {
          ...
          it('should match integers in a map within a range in a nested context', function (done) {
            const args: QueryOptions = { filters: [filter.range('mi', 3, 7, MAPVALUES, new Context().addMapKey('nested'))] }
      ...
      describe('filter.contains()', function () {
        it('should match lists containing an integer', function (done) {
          const args: QueryOptions = { filters: [filter.contains('li', 5, LIST)] }
          verifyQueryResults(args, 'int list match', done)
        })
        ...
        describe('index with cdt context', function () {
          ...
          it('should match lists containing an integer in a nested context', function (done) {
            const args: QueryOptions = { filters: [filter.contains('li', 5, LIST, new Context().addMapKey('nested'))] }
            verifyQueryResults(args, 'nested int list match', done)
          })
        })

        it('should match maps containing an integer value', function (done) {
          const args: QueryOptions = { filters: [filter.contains('mi', 5, MAPVALUES)] }
          verifyQueryResults(args, 'int map match', done)
        })
        ...
        describe('index with cdt context', function () {
          ...
          it('should match maps containing an integer value in a nested context', function (done) {
            const args: QueryOptions = { filters: [filter.contains('mi', 5, MAPVALUES, new Context().addMapKey('nested'))] }
            verifyQueryResults(args, 'nested int map match', done)
          })
        })
        ...
        context('Uses blob Secondary indexes', function () {
          ...
          it('should match lists containing a blob', function (done) {
            const args: QueryOptions = { filters: [filter.contains('lblob', Buffer.from('guava'), LIST)] }
            verifyQueryResults(args, 'blob list match', done)
          })

          it('should match lists containing a blob in a nested context', function (done) {
            const args: QueryOptions = { filters: [filter.contains('lblob', Buffer.from('guava'), LIST, new Context().addMapKey('nested'))] }
            verifyQueryResults(args, 'nested blob list match', done)
          })

          it('should match maps containing a blob value', function (done) {
            const args: QueryOptions = { filters: [filter.contains('mblob', Buffer.from('guava'), MAPVALUES)] }
          ...
          it('should match maps containing a blob value in a nested context', function (done) {
            const args: QueryOptions = { filters: [filter.contains('mblob', Buffer.from('guava'), MAPVALUES, new Context().addMapKey('nested'))] }
            verifyQueryResults(args, 'nested blob map match', done)
          })

          it('should match maps containing a blob key', function (done) {
            const args: QueryOptions = { filters: [filter.contains('mkblob', Buffer.from('guava'), MAPKEYS)] }
          ...
          it('should match maps containing a blob key in a nested context', function (done) {
            const args: QueryOptions = { filters: [filter.contains('mkblob', Buffer.from('guava'), MAPKEYS, new Context().addMapKey('nested'))] }
