...
bool
add_list_set_order_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_list_order order;
...
bool
add_list_sort_op(as_operations* ops, const char* bin, as_cdt_ctx* context,  Local<Object> op, LogInfo* log)
{
...
bool
add_list_append_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_val* val = NULL;
	...
	if (with_policy) {
		if (as_v8_debug_enabled(log)) {
			...
			as_v8_debug(log, "value=%s, order=%i, flags=%i", val_str, policy.order, policy.flags);
			cf_free(val_str);
		}
...
bool
add_list_append_items_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_list* list = NULL;
	...
	if (with_policy) {
		if (as_v8_debug_enabled(log)) {
			...
			as_v8_debug(log, "values=%s, order=%i, flags=%i", list_str, policy.order, policy.flags);
			cf_free(list_str);
		}
...
bool
add_list_insert_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t index;
	...
	if (with_policy) {
		if (as_v8_debug_enabled(log)) {
			char* val_str = as_val_tostring(val);
			as_v8_debug(log, "index=%i, value=%s, order=%i, flags=%i", index, val_str, policy.order, policy.flags);
			cf_free(val_str);
		}
...
bool
add_list_insert_items_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, op, "index", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (with_policy) {
		if (as_v8_debug_enabled(log)) {
			char* list_str = as_val_tostring(list);
			as_v8_debug(log, "index=%i, list=%s, order=%i, flags=%i", index, list_str, policy.order, policy.flags);
			cf_free(list_str);
		}
		as_operations_list_insert_items(ops, bin, context, &policy, index, list);
	} else {
		if (as_v8_debug_enabled(log)) {
			char* list_str = as_val_tostring(list);
...
bool
add_list_pop_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
...
bool
add_list_pop_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, obj, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_remove_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, obj, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_remove_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, obj, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_remove_by_index_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, op, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_remove_by_index_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, op, "index", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (count_defined) {
		...
		as_operations_list_remove_by_index_range(ops, bin, context, index, count, return_type);
	} else {
		as_v8_debug(log, "index=%i, return_type=%i", index, return_type);
		as_operations_list_remove_by_index_range_to_end(ops, bin, context, index, return_type);
	}
...
bool
add_list_remove_by_value_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_val* value = NULL;
...
bool
add_list_remove_by_value_list_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_list* values = NULL;
	...
	as_operations_list_remove_by_value_list(ops, bin, context, values, return_type);

	return true;
}

bool
add_list_remove_by_value_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	bool begin_defined;
	as_val* begin = NULL;
	...
	as_operations_list_remove_by_value_range(ops, bin, context, begin, end, return_type);

	return true;
}

bool
add_list_remove_by_value_rel_rank_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_val* value = NULL;
	if (get_asval_property(&value, op, "value", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (count_defined) {
		...
		as_operations_list_remove_by_value_rel_rank_range(ops, bin, context, value, rank, count, return_type);
	} else {
		if (as_v8_debug_enabled(log)) {
			char* value_str = as_val_tostring(value);
		...
		as_operations_list_remove_by_value_rel_rank_range_to_end(ops, bin, context, value, rank, return_type);
	}
...
bool
add_list_remove_by_rank_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t rank;
	if (get_int64_property(&rank, op, "rank", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_remove_by_rank_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t rank;
	if (get_int64_property(&rank, op, "rank", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (count_defined) {
		...
		as_operations_list_remove_by_rank_range(ops, bin, context, rank, count, return_type);
	} else {
		as_v8_debug(log, "rank=%i, return_type=%i", rank, return_type);
		as_operations_list_remove_by_rank_range_to_end(ops, bin, context, rank, return_type);
	}
...
bool
add_list_clear_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
...
bool
add_list_set_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, obj, "index", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	as_operations_list_set(ops, bin, context, with_policy ? &policy : NULL, index, val);

	return true;
}

bool
add_list_trim_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, obj, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_get_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, obj, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_get_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, obj, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_get_by_index_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, op, "index", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_get_by_index_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, op, "index", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (count_defined) {
		as_v8_debug(log, "index=%i, count=%i, return_type=%i", index, count, return_type);
		as_operations_list_get_by_index_range(ops, bin, context, index, count, return_type);
	} else {
		as_v8_debug(log, "index=%i, return_type=%i", index, return_type);
		as_operations_list_get_by_index_range_to_end(ops, bin, context, index, return_type);
	}
...
bool
add_list_get_by_value_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_val* value = NULL;
	if (get_asval_property(&value, op, "value", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_get_by_value_list_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_list* values = NULL;
	if (get_list_property(&values, op, "values", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	as_operations_list_get_by_value_list(ops, bin, context, values, return_type);

	return true;
}

bool
add_list_get_by_value_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	bool begin_defined;
	as_val* begin = NULL;
	if (get_optional_asval_property(&begin, &begin_defined, op, "begin", log) != AS_NODE_PARAM_OK) {
	...
	as_operations_list_get_by_value_range(ops, bin, context, begin, end, return_type);

	return true;
}

bool
add_list_get_by_value_rel_rank_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	as_val* value = NULL;
	if (get_asval_property(&value, op, "value", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (count_defined) {
		...
		as_operations_list_get_by_value_rel_rank_range(ops, bin, context, value, rank, count, return_type);
	} else {
		if (as_v8_debug_enabled(log)) {
			char* value_str = as_val_tostring(value);
			as_v8_debug(log, "value=%s, rank=%i, return_type=%i", value_str, rank, return_type);
		...
		as_operations_list_get_by_value_rel_rank_range_to_end(ops, bin, context, value, rank, return_type);
	}
...
bool
add_list_get_by_rank_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t rank;
	if (get_int64_property(&rank, op, "rank", log) != AS_NODE_PARAM_OK) {
		return false;
...
bool
add_list_get_by_rank_range_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t rank;
	if (get_int64_property(&rank, op, "rank", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (count_defined) {
		as_v8_debug(log, "rank=%i, count=%i, return_type=%i", rank, count, return_type);
		as_operations_list_get_by_rank_range(ops, bin, context, rank, count, return_type);
	} else {
		as_v8_debug(log, "rank=%i, return_type=%i", rank, return_type);
		as_operations_list_get_by_rank_range_to_end(ops, bin, context, rank, return_type);
	}
...
bool
add_list_increment_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log)
{
	int64_t index;
	if (get_int64_property(&index, op, "index", log) != AS_NODE_PARAM_OK) {
		return false;
	...
	if (with_policy) {
		if (as_v8_debug_enabled(log)) {
			char* value_str = as_val_tostring(value);
			as_v8_debug(log, "index=%i, value=%s, order=%i, flags=%i", index, value_str, policy.order, policy.flags);
			cf_free(value_str);
		}
...
bool
add_list_size_op(as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> obj, LogInfo* log)
{
...
typedef bool (*ListOperation) (as_operations* ops, const char* bin, as_cdt_ctx* context, Local<Object> op, LogInfo* log);
...
add_list_op(as_operations* ops, uint32_t opcode, Local<Object> op, LogInfo* log)
{
	...
	as_v8_debug(log, "Adding list operation %s (opcode %i) on bin %s to operations list, %s CDT context",
...
list_opcode_values()
{
	...
	for (uint32_t i = 0; i < entries; i++) {
		...
		Nan::Set(obj, Nan::New(entry.op_name).ToLocalChecked(), Nan::New(LIST_OPS_OFFSET | i));
	}
