...
static void *prepare(const Nan::FunctionCallbackInfo<Value> &info)
{
	...
	IndexRemoveCommand *cmd =
		new IndexRemoveCommand(client, info[3].As<Function>());
...
static void execute(uv_work_t *req)
{
	...
	as_v8_debug(log, "Executing IndexRemove command: ns=%s, index=%s", cmd->ns,
				cmd->index);
	aerospike_index_remove(cmd->as, &cmd->err, cmd->policy, cmd->ns,
						   cmd->index);
}
