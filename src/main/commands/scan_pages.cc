...
NAN_METHOD(AerospikeClient::ScanPages)
{
	...
	if (info[5]->IsObject()) {
		...
		setup_scan_pages(&scan, info[0], info[1], Nan::Null(), bytes, bytes_size, log);
	}
	else{
		setup_scan_pages(&scan, info[0], info[1], info[2], NULL, 0, log);
	}
	...
	if (pf_defined) {
		...
		status = aerospike_scan_partitions_async(
			client->as, &cmd->err, p_policy, scan, &pf, async_scan_pages_listener,
			su, NULL);
	}
	else {
