...
NAN_METHOD(AerospikeClient::ExistsAsync)
{
	...
	AsyncCommand *cmd =
		new AsyncCommand("Exists", client, info[2].As<Function>());
	...
	if (info[1]->IsObject()) {
		if (readpolicy_from_jsobject(&policy, info[1].As<Object>(), log) !=
			AS_NODE_PARAM_OK) {
	...
	status = aerospike_key_exists_async(client->as, &cmd->err, p_policy, &key,
