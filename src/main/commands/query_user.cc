...
NAN_METHOD(AerospikeClient::QueryUser)
{
	TYPE_CHECK_REQ(info[0], IsString, "User must be a string");
	...
	AsyncCommand *cmd = new AsyncCommand("QueryUser", client, info[2].As<Function>());
	...
	if(info[0]->IsString()){
		...
		CmdErrorCallback(cmd, AEROSPIKE_ERR_PARAM, "username must be a vaild string");
		goto Cleanup;
	}
	...
	if (status != AEROSPIKE_OK) {
		...
		Local<Value> argv[] = { Nan::Null(), Nan::Get(as_users_to_jsobject(&user, 1, log), 0).ToLocalChecked()};
		cmd->Callback(2, argv);
	}
