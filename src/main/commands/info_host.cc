...
static void *prepare(const Nan::FunctionCallbackInfo<v8::Value> &info)
{
	...
	if (info[0]->IsString()) {
		...
		size_t reqlen =
			as_strlcpy(cmd->request, *Nan::Utf8String(info[0].As<String>()),
					   MAX_INFO_REQUEST_LEN);
		if (reqlen > MAX_INFO_REQUEST_LEN) {
			as_v8_info(log,
					   "Info request exceeds max. length (%zu > %i): \"%s...\"",
					   reqlen, MAX_INFO_REQUEST_LEN, cmd->request);
			return CmdSetError(cmd, AEROSPIKE_ERR_PARAM,
							   "Info request exceeds max. length");
		}
	}
	else {
		cmd->request = (char *)"";
	...
	if (host_from_jsobject(info[1].As<Object>(), &cmd->addr, &cmd->port, log) !=
		AS_NODE_PARAM_OK) {
		return CmdSetError(cmd, AEROSPIKE_ERR_PARAM,
						   "Host parameter is invalid");
	}
...
static void execute(uv_work_t *req)
{
	...
	as_v8_debug(log, "Sending info command \"%s\" to cluster host %s:%d",
				cmd->request, cmd->addr, cmd->port);
	aerospike_info_host(cmd->as, &cmd->err, cmd->policy, cmd->addr, cmd->port,
...
static void respond(uv_work_t *req, int status)
{
	...
	if (cmd->IsError()) {
		...
		Local<Value> argv[] = {Nan::Null(), response};
		cmd->Callback(2, argv);
	}
