...
static void respond(uv_work_t * req, int status)
{
    ...
    if ( data->node_err == 1 ) {
        ...
        for ( uint32_t i = 0; i< num_rec; i++) {
            ...
            // status attribute
            result->Set(Nan::New("status").ToLocalChecked(),
                        Nan::New(status));

            // key attribute - should always be sent
            result->Set(Nan::New("key").ToLocalChecked(),
                        key_to_jsobject(key ? key : &record->key, log));

            if( batch_results[i].result == AEROSPIKE_OK ) {

                // metadata attribute
                result->Set(Nan::New("metadata").ToLocalChecked(),
                            recordmeta_to_jsobject(record, log));

                // record attribute
                result->Set(Nan::New("record").ToLocalChecked(),
