...
static void * prepare(ResolveArgs(args))
{
    ...
    if(arglength > 3){
		if(!args[OP_ARG_POS_META]->IsNull() && args[OP_ARG_POS_META]->IsObject() ) {
			// Third argument could be metadata or operate policy.
			// Check for the existence of ttl or gen to make sure that third argument is 
			// metadata.
			if( args[OP_ARG_POS_META]->ToObject()->Has(NanNew("ttl")) || args[OP_ARG_POS_META]->ToObject()->Has(NanNew("gen")))
			{
				meta_present = 1;
