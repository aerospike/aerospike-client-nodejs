...
void ParseSelectBins( AerospikeQuery* query, Local<Object> select)
{
	...
	else 
	{
		...
		return Nan::ThrowError("Bins to be selected is not an array ");
	}   
}
	...
	if ( filter->IsArray() ) { 
		...
		for (int i=0; i < size; i++) {
			...
			if( !bin->IsString() ) {
				...
				return Nan::ThrowError("Bin value is not a string");
			}
				...
				case AS_PREDICATE_RANGE:
					{
						...
						if( v8min->IsNumber()) {
							...
							return Nan::ThrowError("The range value passed is not an integer");
						}
						if( v8max->IsNumber()){
				...
				case AS_PREDICATE_EQUAL:
					{
						...
						if( type == AS_INDEX_NUMERIC) {
                            if( !filter->Get(Nan::New("val").ToLocalChecked())->IsNumber()) {
                                ...
                                return Nan::ThrowError("Querying an integer index with equal predicate - value is not an integer");
                            }
                            ...
                            if( !val->IsString()) {
                                ...
                                return Nan::ThrowError("Querying a string index with equal predicate - value is not a string");
                            }
        ...
        return Nan::ThrowError("filters should be passed as an array");
	} 
}
...
void ParseRecordQSize( int* q_size, Local<Object> qSize, LogInfo* log)
{
	...
	if(qSize->IsNumber()) {
		*q_size = (int) qSize->IntegerValue();
		...
		return Nan::ThrowError("Queue size must be an integer");
	}
}
...
void ParseUDFArgs(QueryScan* queryScan, Local<Object> udf, LogInfo* log, bool isQuery)
{
    ...
    if(ret) {
        ...
        return Nan::ThrowError("Error in parsing the UDF parameters");
    }
...
void ParseScanPriority(as_scan* scan, Local<Object> obj, LogInfo* log)
{
	...
	{   
		...
		return Nan::ThrowError("Scan priority must be of type aerospike.scanPriority");
	}   
}
...
void ParseScanPercent(as_scan* scan, Local<Object> obj, LogInfo* log)
{
	...
	else
	{
		...
		return Nan::ThrowError("Scan percentage is not an integer - expected integer value");
	}
}
...
void ParseScanNobins( as_scan* scan, Local<Value> obj, LogInfo* log)
{
	...
	else
	{
		...
		return Nan::ThrowError("setNobins must be a boolean value");
	}
}

void ParseScanConcurrent(as_scan* scan, Local<Value> obj, LogInfo* log)
{
    Nan::HandleScope socpe;
	...
	else
	{
		...
		return Nan::ThrowError("setConcurrent must be a boolean value");
	}
}
...
NAN_METHOD(AerospikeQuery::New)
{
	...
	if ( !info[0]->IsString()) {
        ...
        return Nan::ThrowError("Namespace to be queried is not a string - expected a string value");
	}
	else {
	...
	if( !info[1]->IsNull() && !info[1]->IsString()) {
        ...
        return Nan::ThrowError("Set to be queried is not a string");
	}
	else {
