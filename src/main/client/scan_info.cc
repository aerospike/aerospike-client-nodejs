...
static void* prepare(ResolveArgs(info))
{
	...
	data->as = client->as;
	data->log = client->log;
	data->scan_id = info[0]->ToInteger()->Value();
	data->callback.Reset(info[2].As<Function>());

	if (info[1]->IsObject()) {
		if (infopolicy_from_jsobject(&data->policy, info[1]->ToObject(), log) != AS_NODE_PARAM_OK) {
...
static void execute(uv_work_t* req)
{
	...
	if (data->param_err) {
		...
		as_v8_debug(log, "Sending scan_info command - scan_id: %lli", data->scan_id);
		aerospike_scan_info(data->as, &data->err, data->p_policy, data->scan_id, &data->scan_info);
	}
}
...
static void respond(uv_work_t* req, int status)
{
	...
	if (data->err.code != AEROSPIKE_OK) {
		...
		argv[0] = err_ok();
		argv[1] = scaninfo_to_jsobject(&data->scan_info, log);
	}

	as_v8_detail(log, "Invoking JS callback for scan_info");
...
NAN_METHOD(AerospikeClient::ScanInfo)
{
	TYPE_CHECK_REQ(info[0], IsNumber, "scan_id must be a number");
